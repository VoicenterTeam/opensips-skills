...
2.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp5642864)

2.2. [Most recently active contributors(1) to this module](#idp5742112)

**List of Examples**

1.1. [Set `cachedb_url` parameter](#idp5528496)

1.2. [Use Redis servers](#idp5531312)

1.3. [Set `connect_timeout` parameter](#idp5536032)

1.4. [Set `connect_timeout` parameter](#idp5541088)

1.5. [Set the `shutdown_on_error` parameter](#idp5546240)

1.6. [Set the `use_tls` parameter](#idp5553664)

1.7. [Set the `ftsearch_index_name` parameter](#idp5560080)

1.8. [Set the `ftsearch_json_prefix` parameter](#idp5566032)

1.9. [Set the `ftsearch_max_results` parameter](#idp5571984)

1.10. [Set the `ftsearch_json_mset_expire` parameter](#idp5578608)

1.11. [Redis Raw Query Examples](#idp5583744)
...
Starting with OpenSIPS **3.6**, the _cachedb\_redis_ module implements the column-oriented cacheDB API functions. This makes it a suitable cacheDB storage in scenarios such as user location _federation_ and _full-sharing_, which require this API to be available.

The implementation makes use of _RedisJSON_ and _RediSearch_ -- these relatively new features are available in Redis Stack Server, instead of the usual Redis Server (Redis OSS project). More documentation is available on the Redis website.
...
*   _If a [use\_tls](#param_use_tls "1.6.5.�use_tls (integer)") is defined, the **tls\_mgm** module will need to be loaded as well_.
...
*   _hiredis:_
    ...
    If TLS connections are enabled via the [use\_tls](#param_use_tls "1.6.5.�use_tls (integer)") modparam, _hiredis_ needs to be compiled with TLS support.
...
Only relevant with _RedisJSON_ and _RediSearch_ server-side support.
...
modparam("cachedb\_redis", "ftsearch\_index\_name", "ix::usrloc")
...
modparam("cachedb\_redis", "ftsearch\_json\_prefix", "userlocation:")
...
modparam("cachedb\_redis", "ftsearch\_max\_results", 100)
...
modparam("cachedb\_redis", "ftsearch\_json\_mset\_expire", 7200)
...
**All remaining contributors**: John Burke ([@john08burke](https://github.com/john08burke)), tcresson, Eddie Fiorentine, Juli�n Moreno Pati�o, Ken Rice, Peter Lemenkov ([@lemenkov](https://github.com/lemenkov)), zhengsh, Jarrod Baumann ([@jarrodb](https://github.com/jarrodb)), Kristian H�gh.
...
**All remaining contributors**: Vlad Patrascu ([@rvlad-patrascu](https://github.com/rvlad-patrascu)), Bogdan-Andrei Iancu ([@bogdan-iancu](https://github.com/bogdan-iancu)), Dan Pascu ([@danpascu](https://github.com/danpascu)), Peter Lemenkov ([@lemenkov](https://github.com/lemenkov)), Kristian H�gh, Juli�n Moreno Pati�o, Jarrod Baumann ([@jarrodb](https://github.com/jarrodb)), jalung, Ezequiel Lovelle.
