...
3.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp5745248)

3.2. [Most recently active contributors(1) to this module](#idp5875376)

**List of Examples**

1.1. [Set `listen` variable](#idp5521136)

1.2. [Set `tls_port` variable](#idp5526512)

1.3. [Set `tls_crlf_pingpong` parameter](#idp5531072)

1.4. [Set `tls_crlf_drop` parameter](#idp5536112)

1.5. [Set `tls_max_msg_chunks` parameter](#idp5540960)

1.6. [Set `cert_check_on_conn_reusage` parameter](#idp5546784)

1.7. [Set `trace_destination` parameter](#idp5553904)

1.8. [Set `trace_on` parameter](#idp5571504)

1.9. [Set `trace_filter_route` parameter](#idp5583840)

1.10. [Set `tls_handshake_timeout` variable](#idp5589632)

1.11. [Set `tls_send_timeout` variable](#idp5594528)

1.12. [Set `tls_async` variable](#idp5599024)

1.13. [Set `tls_async_max_postponed_chunks` parameter](#idp5603680)

1.14. [Set `tls_async_local_connect_timeout` parameter](#idp5608368)

1.15. [Set `tls_async_handshake_timeout` parameter](#idp5613072)
...
_Default value is 5061._

**Example�1.2.�Set `tls_port` variable**

...
modparam("proto\_tls", "tls\_port", 5062)
...
...
...
modparam("proto\_tls", "tls\_crlf\_pingpong", 0)
...
...
Drop CRLF (\\r\\n) ping messages. When this parameter is enabled, the TLS layer drops packets that contains a single CRLF message. If a CRLFCRLF message is received, it is handled according to the _tls\_crlf\_pingpong_ parameter.
...
...
modparam("proto\_tls", "tls\_crlf\_drop", 1)
...
...
...
modparam("proto\_tls", "tls\_max\_msg\_chunks", 8)
...
...
...
modparam("proto\_tls", "cert\_check\_on\_conn\_reusage", 1)
...
...
modparam("proto\_tls", "trace\_destination", "hep\_dest")
...
...
This controls whether tracing for tls is on or not. You still need to define [trace\_destination](#param_tls_trace_destination "1.5.7.�trace_destination (string)")in order to work, but this value will be controlled using mi function [tls\_trace](#mi_tls_trace "1.6.1.� tls_trace").
...
...
modparam("proto\_tls", "trace\_on", 1)
...
...
Define the name of a route in which you can filter which connections will be trace and which connections won't be. In this route you will have information regarding source and destination ips and ports for the current connection. To disable tracing for a specific connection the last call in this route must be **drop**, any other exit mode resulting in tracing the current connection ( of course you still have to define a [trace\_destination](#param_tls_trace_destination "1.5.7.�trace_destination (string)") and trace must be on at the time this connection is opened.
...
**WARNING:** IF [trace\_on](#param_trace_on "1.5.8.�trace_on (int)") is set to 0 or tracing is deactived via the mi command [tls\_trace](#mi_tls_trace "1.6.1.� tls_trace") this route won't be called.
...
...
modparam("proto\_tls", "trace\_filter\_route", "tls\_filter")
...
...
...
modparam("proto\_tls", "tls\_handshake\_timeout", 200) # number of milliseconds
...
...
The send timeout is invoked for all TLS write operations, excluding the handshake process (see: tls\_handshake\_timeout)
...
...
modparam("proto\_tls", "tls\_send\_timeout", 200) # number of milliseconds
...
...
...
modparam("proto\_tls", "tls\_async", 1) # enable async TLS
...
...
If _tls\_async_ is enabled, this specifies the maximum number of SIP messages that can be stashed for later/async writing. If the connection pending writes exceed this number, the connection will be marked as broken and dropped.
...
...
modparam("proto\_tls", "tls\_async\_max\_postponed\_chunks", 16)
...
...
If _tls\_async_ is enabled, this specifies the number of milliseconds that a connect will be tried in blocking mode (optimization). If the connect operation lasts more than this, the connect will go to async mode and will be passed to tls MAIN for polling.
...
...
modparam("proto\_tls", "tls\_async\_local\_connect\_timeout", 200)
...
...
...
modparam("proto\_tls", "tls\_async\_handshake\_timeout", 100)
...
...
*   trace\_mode(optional): set tls tracing on and off. This parameter can be missing and the command will show the current tracing status for this module( on or off ); Possible values:
...
MI FIFO Command Format:
			...
			opensips-cli -x mi tls\_trace on
...
Accumulate as much as possible information (OpenSIPS version, opensips -V output, your OS (uname -a), OpenSIPS logs, network dumps, core dump files, configuration file) and send a mail to [http://lists.opensips.org/cgi-bin/mailman/listinfo/devel](http://lists.opensips.org/cgi-bin/mailman/listinfo/devel)
...
Increase the log level in opensips.cfg (log\_level=4) and watch the log statements in syslog.
...
**All remaining contributors**: Nick Altmann ([@nikbyte](https://github.com/nikbyte)), James Stanley, Juli�n Moreno Pati�o, Peter Lemenkov ([@lemenkov](https://github.com/lemenkov)), Zero King ([@l2dy](https://github.com/l2dy)).
...
**All remaining contributors**: Ionut Ionita ([@ionutrazvanionita](https://github.com/ionutrazvanionita)), Juli�n Moreno Pati�o, Eseanu Marius Cristian ([@eseanucristian](https://github.com/eseanucristian)), Bogdan Chifor, Vlad Paiu ([@vladpaiu](https://github.com/vladpaiu)).
