...
2.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp6931488)

2.2. [Most recently active contributors(1) to this module](#idp7029760)

**List of Examples**

1.1. [Setting the _mode_ module parameter](#idp5998880)

1.2. [Setting the _contact\_id\_insertion_ module parameter](#idp6007088)

1.3. [Setting the _contact\_id\_param_ module parameter](#idp6012704)

1.4. [Setting the _at\_escape\_str_ module parameter](#idp6019632)

1.5. [Setting the _outgoing\_expires_ module parameter](#idp6024704)

1.6. [Setting the _received\_avp_ module parameter](#idp6030992)

1.7. [Setting the _received\_param_ module parameter](#idp6036784)

1.8. [Setting the _extra\_contact\_params\_avp_ module parameter](#idp6042720)

1.9. [Set `attr_avp` parameter](#idp6050096)

1.10. [Setting the _min\_expires_ module parameter](#idp6055376)

1.11. [Setting the _default\_expires_ module parameter](#idp6060256)

1.12. [Setting the _max\_expires_ module parameter](#idp6065104)

1.13. [Setting the _default\_q_ module parameter](#idp6071376)

1.14. [Setting the _tcp\_persistent\_flag_ module parameter](#idp6077072)

1.15. [Setting the _realm\_prefix_ module parameter](#idp6084512)

1.16. [Setting the _case\_sensitive_ module parameter](#idp6090048)

1.17. [Setting the `allow_dup_cseq` parameter](#idp6096544)

1.18. [Setting the `expires_max_deviation` parameter](#idp6145680)

1.19. [Set `max_contacts` parameter](#idp6151376)

1.20. [Setting the _max\_username\_len_ module parameter](#idp6156608)

1.21. [Setting the _max\_domain\_len_ module parameter](#idp6161760)

1.22. [Setting the _max\_aor\_len_ module parameter](#idp6166896)

1.23. [Setting the _max\_contact\_len_ module parameter](#idp6172032)

1.24. [Setting the _retry\_after_ module parameter](#idp6180800)

1.25. [Setting the _gruu\_secret_ module parameter](#idp6185472)

1.26. [Setting the _gruu\_secret_ module parameter](#idp6190240)

1.27. [Setting the `pn_enable` parameter](#idp6240784)

1.28. [Setting the `pn_providers` parameter](#idp6246432)

1.29. [Setting the `pn_ct_match_params` parameter](#idp6254480)

1.30. [Setting the `pn_pnsreg_interval` parameter](#idp6260624)

1.31. [Setting the `pn_trigger_interval` parameter](#idp6268944)

1.32. [Setting the `pn_skip_pn_interval` parameter](#idp6274608)

1.33. [Setting the `pn_refresh_timeout` parameter](#idp6283712)

1.34. [Setting the `pn_enable_purr` parameter](#idp6291920)

1.35. [`_mid_registrar_save_` usage](#idp6380400)

1.36. [`_mid_registrar_lookup_` usage](#idp6495504)

1.37. [`async pn_process_purr()` usage](#idp6553456)
...
*   initial INVITE processing is barely changed, with the _mid\_registrar\_lookup()_ function now additionally returning a value of **2** if the only found contacts were PN-enabled contacts, all which required a Push Notification. This means that PNs have been triggered for each of them and t\_relay() is not required, since they are not reachable until they re-register!
    ...
    Using the event\_routing module, OpenSIPS will transparently fork a new branch from the current INVITE on each re-registration from these contacts within the accepted [pn\_refresh\_timeout](#param_pn_refresh_timeout "1.5.33.�pn_refresh_timeout (integer)")
...
*   _event\_routing_, if [pn\_enable](#param_pn_enable "1.5.27.�pn_enable (boolean)") is set to _true_.
...
### 1.5.17.�`allow_dup_cseq` (boolean)

Some SIP stacks will re-REGISTER using the same Call-ID and CSeq values. While rejecting such requests is consistent with RFC 3261 � 10.3.7, enabling this parameter instructs the mid\_registrar to accept them instead, improving interoperability.

_Default value is _false_ (duplicate CSeq is rejected)._

**Example�1.17.�Setting the `allow_dup_cseq` parameter**

...
# loose RFC 3261 compliance: allow REGISTER requests with duplicate CSeq
modparam("mid\_registrar", "allow\_dup\_cseq", true)
...
		

  

### 1.5.18.�`expires_max_deviation` (integer)
...
**Example�1.18.�Setting the `expires_max_deviation` parameter**
...
**Example�1.19.�Set `max_contacts` parameter**
...
**Example�1.24.�Setting the _retry\_after_ module parameter**

modparam("mid\_registrar", "retry\_after", 30)

  

### 1.5.25.�`disable_gruu` (integer)
...
### 1.5.26.�`gruu_secret` (string)

The string that will be used in XORing when generating temporary GRUUs.

Default value is **"0p3nS1pS"**

**Example�1.26.�Setting the _gruu\_secret_ module parameter**

modparam("mid\_registrar", "gruu\_secret", "my\_secret")

  

### 1.5.27.�`pn_enable` (boolean)

Enable SIP Push Notification support ([_RFC 8599_](https://tools.ietf.org/html/rfc8599)). If enabled, Contact header field URIs which include all [pn\_ct\_match\_params](#param_pn_ct_match_params "1.5.29.�pn_ct_match_params (string)") will be matched against existing bindings using only these parameters. Otherwise, the module will attempt to match them as usual, using the current usrloc [matching\_mode](usrloc#param_matching_mode).

_Default value is **false**._

**Example�1.27.�Setting the `pn_enable` parameter**
...
**Example�1.28.�Setting the `pn_providers` parameter**
...
### 1.5.29.�`pn_ct_match_params` (string)
...
**Example�1.29.�Setting the `pn_ct_match_params` parameter**
...
### 1.5.30.�`pn_pnsreg_interval` (integer)
...
**Example�1.30.�Setting the `pn_pnsreg_interval` parameter**
...
### 1.5.31.�`pn_trigger_interval` (integer)
...
**Example�1.31.�Setting the `pn_trigger_interval` parameter**
...
### 1.5.32.�`pn_skip_pn_interval` (integer)
...
**Example�1.32.�Setting the `pn_skip_pn_interval` parameter**
...
### 1.5.33.�`pn_refresh_timeout` (integer)
...
**Example�1.33.�Setting the `pn_refresh_timeout` parameter**
...
**Example�1.34.�Setting the `pn_enable_purr` parameter**
...
**Example�1.35.�`_mid_registrar_save_` usage**
...
**Example�1.36.�`_mid_registrar_lookup_` usage**
...
**Example�1.37.�`async pn_process_purr()` usage**
