...
4.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp5902192)

4.2. [Most recently active contributors(1) to this module](#idp6027728)

**List of Examples**

1.1. [Set `db_url` parameter](#idp170672)

1.2. [Set `db_table` parameter](#idp5518528)

1.3. [Set `probing_interval` parameter](#idp5523872)

1.4. [Set `probing_method` parameter](#idp5529152)

1.5. [Set `probing_from` parameter](#idp5534128)

1.6. [Set `probing_reply_codes` parameter](#idp5539120)

1.7. [Set `probing_verbose` parameter](#idp5545024)

1.8. [Set the `lb_define_blacklist` parameter](#idp5550464)

1.9. [Set the `fetch_freeswitch_load` parameter](#idp5561936)

1.10. [Set the `initial_freeswitch_load` parameter](#idp5567184)

1.11. [Set `cluster_id` parameter](#idp5593712)

1.12. [Set `cluster_sharing_tag` parameter](#idp5601136)

1.13. [`lb_start` usage](#idp5624688)

1.14. [`lb_next()` usage](#idp5638912)

1.15. [`lb_next()` usage](#idp5649072)

1.16. [`lb_disable_dst()` usage](#idp5656928)

1.17. [`lb_is_destination` usage](#idp5668352)

1.18. [`lb_count_call` usage](#idp5680496)

1.19. [`lb_list` usage](#idp5695808)

1.20. [`lb_status` usage](#idp5702944)
...
Please refer to the Load-Balancer tutorial from the OpenSIPS website: [https://opensips.org/Documentation/Tutorials-LoadBalancing-1-9](https://opensips.org/Documentation/Tutorials-LoadBalancing-1-9).
...
*   _(1)_ - probing only when the destination is in disabled mode (disabling via MI command will competely stop the probing also). The destination will be automatically re-enabled when the probing will succeed next time;
...
*   _script detection_ - by calling from script the lb\_disabled() function after try the destination. In this case, if probing mode for the destination is (1) or (2), the destination will be automatically re-enabled when the probing will succeed.
*   _MI command_ - by calling the lb\_status MI command for disabling (on demand) the destination. If so, the probing and re-enabling of this destination will be completly disabled until you re-enable it again via MI command - this is designed to allow controlled and complete disabling of some destination during maintenance.
...
*   _Dialog_ - Dialog module
    
    _freeswitch_. - only if "fetch\_freeswitch\_stats" is enabled.
    
*   _dialog_ - TM module (only if probing is enabled)
    
*   _clusterer_ - only if "cluster\_id" option is enabled.
    
*   _database_ - one of the DB modules
...
Jeremy Martinez ([@JeremyMartinez51](https://github.com/JeremyMartinez51))
