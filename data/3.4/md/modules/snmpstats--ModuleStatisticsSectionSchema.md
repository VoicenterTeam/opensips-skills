...
3.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp5752304)

3.2. [Most recently active contributors(1) to this module](#idp5852080)

**List of Examples**

1.1. [Setting the `sipEntityType` parameter](#idp173536)

1.2. [Setting the `MsgQueueMinorThreshold` parameter](#idp5522240)

1.3. [Setting the `MsgQueueMajorThreshold` parameter](#idp5526672)

1.4. [Setting the `dlg_minor_threshold` parameter](#idp5531056)

1.5. [Setting the `dlg_major_threshold` parameter](#idp5535536)

1.6. [Setting the `snmpgetPath` parameter](#idp5541184)

1.7. [Setting the `snmpCommunity` parameter](#idp5546400)
...
The SNMPStats module provides a plethora of statistics, some of which are collected by other modules. If the dependent modules are not loaded then those specific statistics will still be returned, but with zeroed values. All other statistics will continue to function normally. This means that the SNMPStats module has no _hard/mandatory_ dependencies on other modules. There are however, _soft_ dependencies, as follows:

*   _usrloc_ - all scalars and tables relating to users and contacts are dependent on the usrloc module. If the module is not loaded, the respective tables will be empty.
    
*   _dialog_ - all scalars relating to the number of dialogs are dependent on the presence of the dialog module. Furthermore, if the module is not loaded, then the openserDialogLimitMinorEvent, and openserDialogLimitMajorEvent alarm will be disabled.
...
IMPORTANT: By default, SNMP loads _mibs_ from _/var/lib/mibs/ietf/_.Keep in mind that you have to copy OpenSIPS _mibs_ wherevere your mibs folder is.
...
Now, start up OpenSIPS in another window. In the snmpd window, you should see a bunch of:
    ...
    agentx\_build: packet built okay
...
Assuming you've configured the trap thresholds in opensips.cfg with something similar to:
    ...
    modparam("snmpstats", "dlg\_minor\_threshold", 500)
    modparam("snmpstats", "dlg\_minor\_threshold", 600)
...
The -f tells the NetSNMP process to not daemonize, and the -Dtrap enables trap debug logs. You should see something similar to the following:
    ...
    Turning on AgentX master support.
    trap: send\_trap 0 0 NET-SNMP-TC::linux
...
When a trap has been received by snmpd, the following will appear in the above output:
    ...
    sent\_trap -1 -1 NET-SNMP-TC::linus
...
OpenSIPS refuses to load the SNMPStats module. Why is it displaying "load\_module: could not open module snmpstats.so"?

On some systems, you may receive the following error at stdout or the log files depending on the configuration.

    ERROR: load\_module: could not open module </usr/local/lib/opensips/modules/snmpstats.so>:
...
Alternatively, you may prefix your startup command with:
    ...
    LD\_LIBRARY\_PATH=/path/noted/in/step/one/above
		

For example, on my system I ran:

			LD\_LIBRARY\_PATH=/usr/local/lib /etc/init.d/opensips start
...
**All remaining contributors**: Juli�n Moreno Pati�o, Klaus Darilion, Konstantin Bokarius, Peter Lemenkov ([@lemenkov](https://github.com/lemenkov)), Ionut Ionita ([@ionutrazvanionita](https://github.com/ionutrazvanionita)), Sergio Gutierrez, Vlad Patrascu ([@rvlad-patrascu](https://github.com/rvlad-patrascu)), Edson Gellert Schubert, Walter Doekes ([@wdoekes](https://github.com/wdoekes)).
...
**All remaining contributors**: Sergio Gutierrez, Henning Westerholt ([@henningw](https://github.com/henningw)), Klaus Darilion, Anca Vamanu, Daniel-Constantin Mierla ([@miconda](https://github.com/miconda)), Konstantin Bokarius, Edson Gellert Schubert, Jesus Rodrigues, Jeffrey Magder.
