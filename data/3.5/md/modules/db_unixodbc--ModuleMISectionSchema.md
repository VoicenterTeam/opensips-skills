...
3.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp30746240)

3.2. [Most recently active contributors(1) to this module](#idp30844528)

**List of Examples**

1.1. [Set the “auto\_reconnect” parameter](#idp25397488)

1.2. [Set the “use\_escape\_common” parameter](#idp25388160)

## Chapter�1.�Admin Guide

## 1.1.�Overview

This module allows to use the unixodbc package with OpenSIPS. It have been tested with mysql and the odbc connector, but it should work also with other database. The auth\_db module works.
...
....
loadmodule "/usr/local/lib/opensips/modules/db\_unixodbc.so"
....
...
loadmodule "/usr/local/lib/opensips/modules/auth\_db.so"
modparam("usrloc", "db\_mode", 2)
modparam("auth\_db", "calculate\_ha1", yes)
...
....
modparam("usrloc|auth\_db", "db\_url", 
    "unixodbc://opensips:opensipsrw@localhost/my\_dsn")
....

replacing my\_dsn with the correct value.

HINT: if unixodbc don't want to connect to mysql server, try restarting mysql server with:

shell>safe\_mysqld --user=mysql --socket=/var/lib/mysql/mysql.sock
