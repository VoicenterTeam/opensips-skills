...
2.1. [Top contributors by DevScore(1), authored commits(2) and lines added/removed(3)](#idp5600928)

2.2. [Most recently active contributors(1) to this module](#idp5700560)

**List of Examples**

1.1. [Set `db_url` parameter](#idp163344)

1.2. [Set `rooms_table` parameter](#idp167072)

1.3. [Set `members_table` parameter](#idp172048)

1.4. [Set `hash_size` parameter](#idp5523392)

1.5. [Set `imc_cmd_start_char` parameter](#idp5527824)

1.6. [Set `outbound_proxy` parameter](#idp5532544)

1.7. [Usage of `imc_manager()` function](#idp5538400)

1.8. [List of commands](#idp5556544)
...
A command is identified by the starting character. A command must be written in one line. By default, the starting character is '#'. You can change it via "imc\_cmd\_start\_char" parameter.
...
Before running OpenSIPS with IMC, you have to setup the database tables where the module will store the data. For that, if the tables were not created by the installation script or you choose to install everything by yourself you can use the imc-create.sql SQL script in the database directories in the opensips/scripts folder as template. You can also find the complete database documentation on the project webpage, [https://opensips.org/docs/db/db-schema-devel.html](https://opensips.org/docs/db/db-schema-devel.html).
...
**All remaining contributors**: Sergio Gutierrez, Konstantin Bokarius, Peter Lemenkov ([@lemenkov](https://github.com/lemenkov)), Edson Gellert Schubert, Walter Doekes ([@wdoekes](https://github.com/wdoekes)).
...
**All remaining contributors**: Konstantin Bokarius, Edson Gellert Schubert, Henning Westerholt ([@henningw](https://github.com/henningw)), Anca Vamanu, Elena-Ramona Modroiu.
