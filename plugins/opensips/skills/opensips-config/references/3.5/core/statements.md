...
<!-- generated-from: data/3.5/core/statements.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: statement -->

Reference for OpenSIPs 3.5 script statements. Read this file when assembling control-flow constructs (if, switch, while, return, etc.) or confirming where a given statement is usable.
...
- `collection` *(indexed pseudo-variable, required)* — The indexed variable or pseudo-variable array to iterate over.
- `iterator` *(pseudo-variable, required)* — The variable that holds the current item in the iteration.

**Usable from:** routing logic
...
IF-ELSE statement. The 'expr' should be a valid logical expression. The logical operators that can be used in the logical expressions: == - equal, != - not equal, =~ - regular expression matching, !~ - regular expression not-matching, > - greater, >= - greater or equal, < - less, <= - less or equal, && - logical AND, || - logical OR, ! - logical NOT, [ ... ] - test operator - inside can be any arithmetic expression.
...
- `expr` *(logical expression, required)* — A valid logical expression to evaluate.
...
- `pseudo-variable` *(pseudo-variable, required)* — The pseudo-variable to test the value of.
...
- `expr` *(logical expression, required)* — A valid logical expression to evaluate for the loop condition.
