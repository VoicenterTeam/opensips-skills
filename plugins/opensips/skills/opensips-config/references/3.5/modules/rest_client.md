# rest_client Module Reference
<!-- generated-from: data/3.5/modules/rest_client.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 rest_client module. Read this file when configuring or debugging the rest_client module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The _rest_client_ module provides a means of interacting with an HTTP server by doing RESTful queries, such as GET, POST and PUT.
...
Use a specific HTTP version for all requests.
...
**Example.** /home/opensips/ca_certificates.
...
- `txt` *(string, required)* — The contents of _txt_ should adhere to the specification for HTTP headers (ex. Field: Value)

**Usable from:** any route

**Example.** Example of querying a REST service requiring additional headers.
...
- `body_pv` *(var, required)* — output variable which will hold the body of the HTTP response.
...
- `-3` — Transfer Timeout (the curl_timeout was exceeded before the last byte was received). The _retcode_pv_ may be set to 200 or 0, depending whether a 200 OK was received or not. If it was, the _body_pv_ will contain partially downloaded data, use at your own risk! (we recommend you only use this data for logging / debugging purposes)
...
**Example.** Example of querying a REST service to get the credit of an account.
...
Force a specific TLS domain to be used at most once, during the next GET/POST/PUT request. Refer to the tls_mgm module for additional info regarding TLS client domains. If using this function, you must also ensure that tls_mgm is loaded and properly configured.

**Parameters:**

- `tls_client_domain` *(string, required)* — The name of the TLS client domain to use.

**Usable from:** any route

**Example.** Example of using a specific TLS client domain for a request.
...
Perform a blocking HTTP POST on the given _url_. Note that the _send_body_ parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.

**Parameters:**

- `recv_body_pv` *(var, required)* — output variable which will hold the body of the HTTP response.
- `recv_ctype_pv` *(var, optional)* — output variable which will contain the value of the "Content-Type" header of the response
- `retcode_pv` *(var, optional)* — output variable which will retain the status code of the HTTP response. A **0** status code value means no HTTP reply arrived at all.
- `send_body` *(string, required)* — The request body. Note that the _send_body_ parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.
- `send_ctype` *(string, optional)* — The MIME Content-Type header for the request.
- `url` *(string, required)* — The URL to query.
...
**Example.** Creating a resource using a RESTful service with an HTTP POST request.
...
Perform a blocking HTTP PUT on the given _url_. Similar to rest_post(), the _send_body_pv_ parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.
...
- `recv_ctype_pv` *(var, optional)* — output variable which will contain the value of the "Content-Type" header of the response
- `retcode_pv` *(var, optional)* — output variable which will retain the status code of the HTTP response. A **0** status code value means no HTTP reply arrived at all.
- `send_body` *(string, required)* — The request body. Similar to rest_post(), the _send_body_pv_ parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.
- `send_ctype` *(string, optional)* — The MIME Content-Type header for the request.
- `url` *(string, required)* — The URL to query.
...
**Example.** Creating/Updating a resource using a RESTful service with an HTTP PUT request.
...
### Setting the `curl_timeout` parameter
...
...
```
### Setting the `max_async_transfers` parameter
...
...
```
### Setting the `max_transfer_size` parameter
...
...
```
### Setting the `ssl_verifypeer` parameter
...
...
```
### Setting the `ssl_verifyhost` parameter
...
...
```
### Setting the `ssl_capath` parameter
...
...
```
### Setting the `curl_http_version` parameter
...
...
```
### Setting the `enable_expect_100` parameter
...
...
```
### Setting the `no_concurrent_connects` parameter
...
...
```
### Setting the `curl_conn_lifetime` parameter
