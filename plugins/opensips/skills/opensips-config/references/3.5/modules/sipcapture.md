...
<!-- generated-from: data/3.5/modules/sipcapture.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 sipcapture module. Read this file when configuring or debugging the sipcapture module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `database module` — Required for storing SIP messages
...
*   _none_ - don't go through the script; do directly sip_capture();
...
- `chunk_id` *(string, required)* — same meaning as the _chunk_id_ in [hep_set()](#func_hep_set "1.4.3. hep_set(chunk_id, chunk_data, [data_type], [vendor_id])").
...
- `chunk_id` *(string, required)* — same meaning as in [hep_set()](#func_hep_set "1.4.3. hep_set(chunk_id, chunk_data, [data_type], [vendor_id])")
- `data_type` *(string, required)* — same meaning as in [hep_set()](#func_hep_set "1.4.3. hep_set(chunk_id, chunk_data, [data_type], [vendor_id])"); can miss if it's a generic chunk
...
- `data_type` *(string, optional)* — data type of the data in the chunk. It can have the following values:
  ...
  - `utf8-string`
  - `octet-string`
- `vendor_id` *(string, optional)* — there are some vendor ids already defined; check [hep proto docs](http://hep.sipcapture.org/hepfiles/HEP3_rev11.pdf) for more details.
...
*   _proto_family_ - can be AF_INET/AF_INET6
    
*   _proto_id_ - it's PROTO_HEP since you receive the message as hep.
    
*   _src_ip_ - IPv4/IPv6 address, depending on the proto_family, of the sending node.
    
*   _dst_ip_ - IPv4/IPv6 address, depending on the proto_family, of the receiving node(OpenSIPS hep interface ip on which the message was received).
    
*   _src_port_ - Sending node port.
    
*   _dst_port_ - Receiving port(OpenSIPS hep interace port on which the message was received).
...
**Returns:** Status of the SIP message capturing (as string “on” or “off” )
...
Sets the table name where to store the SIP messages, including strftime-like suffixes.
...
Sets the table name where to store packets captured with report_capture function.
...
Enables or disables capture globally.
...
Enables or disables capture of HEP.
...
Sets the maximum number of 'INSERT' queries of captured packets to be done in the same time.
...
Enables or disables IPIP capturing.
...
Enables or disables monitoring/mirroring port capturing.
...
Sets the listen IP address of RAW socket for IPIP capturing and defines port/portrange.
...
Sets the name of the interface to bind on the raw socket.
...
Defines how much children must be created to listen the raw socket.
...
Enables or disables promiscuous mode on the raw socket.
...
Activates Linux Socket Filter (LSF based on BPF) on the mirroring interface.
...
Sets the name of the capture node.
...
Saves the message into the database using report_capture.
...
Relays a message statefully to destination indicated in current URI.
...
Breaks hep route execution and resumes into the main request route.
...
Saves the message inside the database asynchronously.
...
Demonstrates accessing layer 3 and 4 information about the node from where the hep message was received.
...
Demonstrates checking the version of the hep packet received.
