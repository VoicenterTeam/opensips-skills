...
<!-- generated-from: data/3.5/modules/osp.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 osp module. Read this file when configuring or debugging the osp module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- 0 - does not report any network ID
- 1 - reports source network ID
- 2 - reports destination network ID
- 3 - reports both source and destination network IDs
...
- 0 - will not use the number portability parameters
- 1 - uses the number portability parameters in the Request URI when these parameters exist
...
These parameter is used to tell the OSP module which AVP is used to store source user ID information. All pseudo variables are described in https://opensips.org/Resources/DocsCoreVar.
...
- 0 - will not validate the call id in the peering token
- 1 - validates that the call id in the SIP INVITE message matches the call id in the peering token
...
- `release` *(integer, required)* — Indicates which device releases the call.
  - `0`
  - `1`
...
- `-1` — The address of the called party must be a valid E164 number, otherwise this function returns -1. ... on errors (peering servers are not available, authentication failed or there is no route to destination or the route is blocked)
