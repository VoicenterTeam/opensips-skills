...
<!-- generated-from: data/3.5/modules/launch_darkly.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 launch_darkly module. Read this file when configuring or debugging the launch_darkly module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `ldserverapi` — The following libraries or applications must be installed before running OpenSIPS with this module loaded. _ldserverapi_ must be compiled and installed from the official [GITHUB repository](https://github.com/launchdarkly/c-server-sdk).
...
**Notes:** Note that these log produced by the LD library (according to this ld_log_level) will be further subject to filtering according to the overall OpenSIPS log_level.
...
**Notes:** Note: if there are no flag checkings to be performed, the re-init may be attempted longer than \`re_init_interval\`.
...
**Notes:** This is a mandatory parameter.
...
- `1` — flag was found TRUE
- `-1` — flag was found FALSE
- `2` — fallback TRUE (error occurred)
- `-2` — fallback FALSE (error occurred)
...
Demonstrates setting the `sdk_key` parameter.
...
Demonstrates setting the `ld_log_level` parameter.
...
Demonstrates setting the `connect_wait` parameter.
...
Demonstrates setting the `re_init_interval` parameter.
...
Demonstrates the usage of the `ld_feature_enabled()` function.
