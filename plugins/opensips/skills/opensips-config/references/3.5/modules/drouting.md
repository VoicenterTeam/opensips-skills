...
<!-- generated-from: data/3.5/modules/drouting.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 drouting module. Read this file when configuring or debugging the drouting module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Example.** Set the `cluster_probing_mode` parameter.
...
The name of the sharing tag (as defined per clusterer modules) to control which node is responsible for perform the self-triggered actions in the module. Such actions may be the gateway probing (see also the cluster_probing_mode parameter) or sharing the gateway/carrier status changes. If defined, only the node with active status of this tag will perform the actions (pinging and sharing status).
...
**Example.** Set the `drg_domain_col` parameter.
...
**Example.** Set the `drg_grpid_col` parameter.
...
**Example.** Set the `drg_user_col` parameter.
...
**Example.** Set the `gw_id_avp` parameter.
...
**Example.** Set the `gw_priprefix_avp` parameter.
...
Variable which will store the name of the name partition when _wildcard(*) operatior is used. _Use_partitions_ must be set in order to use this parameter.
...
**Example.** Set the `rule_id_avp` parameter.
...
**Example.** Set the `rule_prefix_avp` parameter.
...
**Example.** Set the `ruri_avp` parameter.
...
**Example.** Set the `use_domain` parameter.
...
Function to trigger routing of the message according to the rules in the database table and the configured parameters. If you set `use_partitions` to 1 the **partition** last parameter becomes mandatory. All parameters are optional. Any of them may be ignored, provided the necessary separation marks "," are properly placed.
...
Marks as disabled the last destination that was used for the current call. The disabling done via this function will prevent the destination to be used for usage from now on. The probing mechanism can re-enable this peer (see the probing section in the beginning). If you set `use_partitions` parameter to 1 you must supply the "partition" parameter to instruct on the partition where the gateway has been defined.
...
The function checks if the SIP URI hostname part stored inside the "src_pv" pseudo-variable is a gateway from a certain group. This function does not change anything in the message. If you set `use_partitions` parameter to 1 you must supply the "partition" parameter to instruct on the partition where the gateway has been defined.
...
The function tries to match/check the given number against the rules from the database. If you set `use_partitions` to 1 the **partition** last parameter becomes mandatory.
...
- `true` — the destination of the current request (destination URI or Request URI) points (as IP) to one of the gateways
...
**Example.** use_partitions is enabledt.
...
Function to trigger the direct routing to a given set carriers (one or more). So, the routing is not done prefix based, but carrier based (call will be sent to the GWs of that carrier, based on carrier policy). If you set `use_partitions` parameter to 1 you must supply the "partition" parameter also (where the carrier are to be found).
...
Function to trigger the direct routing to a given gateway (or list of gateways). Attributes and per-gw processing will be available. If you set `use_partitions` parameter to 1 you must supply the "partition" parameter to instruct on the partition where the gateway has been defined.

**Parameters:**

- `carrier_attrs_var` *(var, optional)* — an output writable variable which will be populated with the attributes of this carrier. NOTE: the first carrier pointing to the GW(s) will be considered!
- `gw_attrs_var` *(var, optional)* — an output writable variable which will be populated with the attributes of the currently matched gateway.
...
- `partition_name` *(string, optional)* — Required if use_partitions is set to 1.
...
**Example.** Get status of carrier 'CR1' when use_partitions is 0.
...
**Example.** Set status of carrier 'CR1' to enabled when use_partitions is 0.

```opensips-cli
$ opensips-cli -x mi dr_carrier_status carrier_id=CR1 status=1
```

**Example.** Verify status of carrier 'CR1' when use_partitions is 0.
...
**Example.** Get status of carrier 'CR1' in partition 'my_partition' when use_partitions is 1.
...
**Example.** Set status of carrier 'CR1' in partition 'partition_1' to enabled when use_partitions is 1.

```opensips-cli
$ opensips-cli -x mi dr_carrier_status partition_name=partition_1 carrier_id=CR1 status=1
```

**Example.** Verify status of carrier 'CR1' in partition 'partition_3' when use_partitions is 1.
...
**Example.** Get current probing status.
...
**Example.** Disable gateway probing.

```opensips-cli
$ opensips-cli -x mi dr_enable_probing 0
```

**Example.** Verify probing status is disabled.
...
**Example.** Get status of gateway 2 when use_partitions is 0.
...
**Example.** Set status of gateway 2 to disabled when use_partitions is 0.

```opensips-cli
$ opensips-cli -x mi dr_gw_status gw_id=2 status=0
```

**Example.** Verify status of gateway 2 when use_partitions is 0.
...
**Example.** Check status of gateway 3 when use_partitions is 0.
...
**Example.** Get status of gateway 'my_gw' in partition 'part_1' when use_partitions is 1.
...
**Example.** Set status of gateway 'my_gw' in partition 'part_1' to disabled when use_partitions is 1.

```opensips-cli
$ opensips-cli -x mi dr_gw_status partition_name=part_1 gw_id=my_gw status=0
```

**Example.** Verify status of gateway 'my_gw' in partition 'part_1' when use_partitions is 1.
...
**Example.** Check status with invalid status value when use_partitions is 1.
...
- `group_id` *(string, optional)* — The group id of the rules to check against.
- `number` *(string, required)* — The number to test against.
- `partition_name` *(string, optional)* — Required if use_partition is set to 1.

**Example.** Test routing for a number in a specific partition and group.
...
- `partition_name` *(string, optional)* — If use_partition is set to 1: if not provided all the partitions will be reloaded, otherwise just the partition given as parameter will be reloaded.

**Example.** Reloads rules for partition 'part_1'.
...
- `partition_name` *(string, optional)* — If use_partition is set to 1: if not provided the function will list the time of the last update for every partition. Otherwise, the function will list the time of the last reload for the given partition.

**Example.** Get reload status when use_partitions is 0.
...
**Example.** Get reload status for all partitions when use_partitions is 1.
...
**Example.** Get reload status for specific partition 'part_test' when use_partitions is 1.
...
The database url. Default value is “NULL”.
...
The name of the db table storing gateway addresses. Default value is “dr_gateways”.
...
The name of the db table storing routing rules. Default value is “dr_rules”.
...
The name of the db table storing groups. Default value is “dr_groups”.
...
The name of the db table storing definitions of the carriers that will be used directly by the routing rules. Default value is “dr_carriers”.
...
The name of the avp for storing Request URIs to be later used (alternative destiantions for the current one). Default value is “$avp(___dr_ruri__)” if use_partitions parameter is 0 or “$avp(___dr_ruri__partition_name)” where partition_name is the name of the partition containing the AVP (as fetched from the database) if use_partitions parameter is 1.
...
The name of the avp for storing the id of the current selected gateway/destination - once a new destination is selected (via the use_next_gw() function), the AVP will be updated with the ID of the new selected gateway/destination. Default value is “$avp(___dr_gw_id__)” if use_partitions parameter is 0 or “$avp(___dr_gw_id__partition_name)” where partition_name is the name of the partition containing the AVP (as fetched from the database) if use_partitions parameter is 1.
...
The name of the avp for storing the PRI prefix of the current selected destination/gateway - once a new destination is selected (via the use_next_gw() function), the AVP will be updated with the PRI prefix of the new used destination. Default value is “NULL”.
...
The name of the avp for storing the id of the current matched routing rule (see dr_rules table). Default value is “NULL”.
...
The actual prefix that matched the routing rule (the part from RURI username that matched the routing rule). Default value is “NULL”.
...
AVP to be populate with the ID string for the carrier the current GW belongs to. Default value is “NULL”.
...
Defines a blacklist based on a list of GW types - the blacklist will be populated with the IPs (no port, all protocols) of the GWs having the specified types. If partitions are used, prefix the blacklist definition string with the name of the partition followed by ":" separator. Multiple instances of this param are allowed. Default value is “NULL”.
...
Group to be used if the caller (FROM user) is not found in the GROUP table. Default value is “NONE”.
...
Force DNS resolving of GW/destination names (if not IPs) during startup. If not enabled, the GW name will be blindly used during routing. Default value is “1 (enabled)”.
...
Specifies whether the state column should be loaded at startup and flushed during runtime or not. Default value is “1” (enabled).
...
If enabled, the module will not allow do run multiple dr_reload MI commands in parallel (with overlapping) Any new reload will be rejected (and discarded) while an existing reload is in progress. If you have a large routing set (millions of rules/prefixes), you should consider disabling concurrent reload as they will exhaust the shared memory (by reloading into memory, in the same time, multiple instances of routing data). Default value is “0 (disabled)”.
...
How often (in seconds) the probing of a destination should be done. If set to 0, the probing will be disabled as functionality (for all destinations) Default value is “30”.
...
The SIP method to be used for the probing requests. Default value is “"OPTIONS"”.
...
The FROM SIP URI to be advertised in the SIP probing requests. Default value is “"sip:prober@localhost"”.
...
A comma separted list of SIP reply codes. The codes defined here will be considered as valid reply codes for probing messages, apart for 200. Default value is “NULL”.
...
A socket description [proto:]host[:port] of the local socket (which is used by OpenSIPS for SIP traffic) to be used (if multiple) for sending the probing messages from. For probing gateway the highest priority has socket from gateway configuration in dr_gateways table. Then socket from global probing_socket parameter and the lowest priority is default behaviour with auto selected socket wich OpenSIPS listens on. Default value is “NULL”.
...
This parameter controls the gateway filtering during DB loading, or which gateways are loaded or not into memory depending on the configured socket they have. The supported filtering modes are: "all", "ignore", "matched-only". Default value is “"all"”.
...
The ID of the cluster the module is part of. The clustering support is used in drouting module for two purposes: for sharing the status of the gateways/carriers and for controlling the pinging to gateways. Default value is “0 (none)”.
...
The name of the sharing tag (as defined per clusterer modules) to control which node is responsible for perform the self-triggered actions in the module. Such actions may be the gateway probing (see also the cluster_probing_mode parameter) or sharing the gateway/carrier status changes. If defined, only the node with active status of this tag will perform the actions (pinging and sharing status). Default value is “empty (none)”.
...
This paramter controls how the probing/pinging should be done when using the clustering support. It is about which node in the cluster pings which gateway/destination. Default value is “"all"”.
...
Flag to configure whether to use domain match when querying database for user's routing group. Default value is “1”.
...
The name of the column in group db table where the username is stored. Default value is “username”.
...
The name of the column in group db table where the domain is stored. Default value is “domain”.
...
The name of the column in group db table where the group id is stored. Default value is “groupid”.
...
Flag to configure whether to use partitions for routing. If this flag is set then the db_partitions_url and db_partitions_table variables become mandatory. Default value is “0”.
...
The url to the database containing partition-specific information. (partition-specific information includes partition name, url to the database where information about the partition is preserved, the names of the tables in which it is preserved and the AVPs that can be accessed using the .cfg script). The use_partitions parameter must be set to 1. Default value is “"NULL"”.
...
The name of the table containing partition definitions. To be used with use_partitions and db_partitions_url. Default value is “dr_partitions”.
...
Variable which will store the name of the name partition when wildcard(*) operatior is used. Use_partitions must be set in order to use this parameter. NOTE: The variable must be WRITABLE! Default value is “null(not used)”.
...
List of ASCII (0-127) characters to be additionally accepted in the prefixes. By default only '0' - '9' chars (digits) are accepted. Default value is “NULL”.
...
A set of extra characters to be allowed in both Gateway and Carrier unique string identifiers, on top of alphanumeric characters. Default value is “_-.”.
...
This parameter offers a dynamic, SQL-based way of building a set of dr_rules-compatible table names, to be each loaded and then merged into a single "dr_rules" table, for any given partition. The syntax of the parameter is: "token : query", where token is a special name given to a "dr_rules" table, so OpenSIPS can match it against the custom queries defined using this parameter. This parameter may be set multiple times (each definition creates a new mapping).
...
null
### dr_gw_status usage when use_partitions is set to 0
...
null
### dr_gw_status usage when use_partitionsis set to 1
...
null
### dr_carrier_status usage when use_partitions is 0
...
null
### dr_carrier_status usage when use_partitions is 1
...
null
### dr_reload_status usage when use_partitions is 0
...
null
### dr_reload_status usage when use_partitions is 1
