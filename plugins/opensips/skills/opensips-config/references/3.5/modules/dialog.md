...
<!-- generated-from: data/3.5/modules/dialog.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 dialog module. Read this file when configuring or debugging the dialog module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `RR` — Record-Route module, if Dialog ID matching is used in non Topo Hiding cases (optional)
...
- `name` *(string, required)* — The name of the attribute to fetch.
- `val` *(var, required)* — The variable to store the fetched value.
...
- `true` — if dialogs are found and values written
...
- `true` — if dialog variables are found and returned
- `false` — if dialog is not found or error occurs

**Usable from:** ANY_ROUTE

**Example.** Example 1.59. `get_dialog_vals` usage.
...
- `true` — if dialogs are found
- `false` — if no dialogs found or error

**Usable from:** ANY_ROUTE

**Example.** Example 1.61. `get_dialog_vals` usage.
...
**Example.** Example 1.60. `get_dialog_vals` usage.
...
Returns true if the dialog flag named flag is set. The dialog flags are dialog persistent and they can be accessed (set and test) for all requests belonging to the dialog.
...
**Example.** Example 1.62. `load_dialog_ctx` usage.
...
Resets the dialog flag named flag to false. The dialog flags are dialog persistent and they can be accessed (set and test) for all requests belonging to the dialog.
...
Attaches to the dialog the value from the variable val under the name name. The values attached to dialogs are dialog persistent and they can be accessed (read and write) for all requests belonging to the dialog.
...
- `name` *(string, required)* — The name under which the value is attached.
- `val` *(var, required)* — The variable containing the value to attach.
...
Atomically checks if the dialog flag named flag is equal to value. If true, changes the value with the opposite one. This operation is done under the dialog lock.
...
**Example.** For usage example, see the load_dialog_ctx().
...
**Example.** Sync dialogs for sharing tag vip1
...
**Example.** Sync dialogs from database
...
- `from_tag` *(string, optional)* — fromtag (as per initial request) of the dialog to be listed. entry (cannot be present without the callid parameter)
- `index` *(integer, optional)* — offset where the dialog listing should start.

**Returns:** List of dialogs and their descriptions.
...
**Returns:** List of dialogs with their descriptions, associated context, and values.
...
- `DID` *(string, required)* — dialog identifier. Can be either the $DLG_did or the actual Call-ID. Can be multiple.
...
**Example.** Push or update a dialog value for multiple dialog IDs
...
**Returns:** Returns the status code and reason of the last reply received for either the challenge or normal mode.
...
**Returns:** List of dialog profiles and whether they have an associated value (1 or 0).
...
**Example.** Terminate all dialogs in the inboundCalls profile
...
**Returns:** The number of dialogs belonging to the profile.

**Example.** Get the size of the inboundCalls profile
...
**Returns:** List of values belonging to the profile along with their count, or a total count if values are not supported.

**Example.** List values and counts for the inboundCalls profile
...
**Returns:** List of dialogs belonging to the specified profile.

**Example.** List dialogs for the inboundCalls profile
...
- `db_id` *(integer)* — the integer representation of the dialog id, as it is stored in the database dlg_id field.
...
List of names (alphanumerical/-/_) for profiles with values. Flags /b or /s allow sharing profiles between OpenSIPS instances using the clusterer module or a CacheDB backend, respectively.
...
List of names (alphanumerical/-/_) for profiles without values. Flags /b or /s allow sharing profiles between OpenSIPS instances using the clusterer module or a CacheDB backend, respectively.
