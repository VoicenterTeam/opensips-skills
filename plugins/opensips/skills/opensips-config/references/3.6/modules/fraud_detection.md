...
<!-- generated-from: data/3.6/modules/fraud_detection.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 fraud_detection module. Read this file when configuring or debugging the fraud_detection module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Show the current statistics for all dials of a user to a prefix. NOTE: Since the fraud statistics are refreshed on-the-fly, as check_fraud() is called, **this function will return stale data** if check_fraud() has not been called at least once for the (user, prefix) pair within a newly matching time interval!
...
- `value` *(integer)* — the current value of the parameter.
- `threshold` *(integer)* — the warning threshold value.
- `user` *(string)* — the user who initiated the call.
- `called_number` *(string)* — the number that was called.
- `rule_id` *(integer)* — the id of the fraud rule that matched when the call was initiated
- `profile_id` *(integer)* — the profile id used
### `E_FRD_WARNING`
...
The column's name in the database storing the the start time of the interval in which the rule will match. The time needs to be specified as string using the format: “HH:MM”
...
The column's name in the database storing the the end time of the interval in which the rule will match. The time needs to be specified as string using the format: “HH:MM”
