...
<!-- generated-from: data/3.6/modules/osp.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 osp module. Read this file when configuring or debugging the osp module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- 0 (no crypto hardware is present)
- 1 (crypto hardware is used)
...
Instructs the OSP module to use the defined AVP to pass the Identity related values.
...
The retry_delay (integer) parameter defines the time, in seconds, between retrying connection attempts to an OSP peering server. After exhausting all peering servers the OSP module will delay for this amount of time before resuming connection attempts.
...
Used to tell the OSP module which AVP is used to store source service provider information.
...
The ssl_lifetime (integer) parameter defines the lifetime, in seconds, of a single SSL session key. Once this time limit is exceeded, the OSP module will negotiate a new session key. Communication exchanges in progress will not be interrupted when this time limit expires.
...
Used to tell the OSP module which AVP is used to store source user group information.
...
Used to tell the OSP module which AVP is used to store source user ID information.
...
- `direction` *(integer, required)* — Specifies which device released the call.
  - `0`
  - `1`
...
- `1` — If the transaction was accepted by the peering server, the uri is being rewritten
- `-1` — The address of the called party is not a valid E164 number OR on errors (peering servers are not available, authentication failed or there is no route to destination or the route is blocked)
