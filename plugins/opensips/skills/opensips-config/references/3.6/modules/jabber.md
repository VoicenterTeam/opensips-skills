...
<!-- generated-from: data/3.6/modules/jabber.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 jabber module. Read this file when configuring or debugging the jabber module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Sets the SQL URL of the database.
...
Sets the IP or hostname of the Jabber server.
...
Sets the port number of the Jabber server.
...
Sets the format for the Jabber domain.
...
Sets the address in whose behalf the INFO and ERROR messages are sent.
...
Sets the maximum jobs per worker.
...
Sets the cache time of a Jabber connection.
...
Sets the time to keep a SIP message.
...
Sets the time between expired Jabber connections checking.
...
Sets the time between checking the status of JabberGW workers.
...
Sets the presence priority for Jabber gateway.
...
Converts a SIP MESSAGE message to a Jabber message and sends it to the Jabber server.
...
Registers to the Jabber server.
...
Logs off from the Jabber server.
