...
<!-- generated-from: data/3.6/modules/cachedb_redis.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 cachedb_redis module. Read this file when configuring or debugging the cachedb_redis module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `hiredis` — Must be installed before running OpenSIPS with this module loaded. If TLS connections are enabled, hiredis needs to be compiled with TLS support.
...
Only relevant with _RedisJSON_ and _RediSearch_ server-side support. A global index name to be used for all internal JSON full-text search operations. Future extensions may add, e.g., a connection-level index name setting.

*Default value is idx:usrloc.*

**Notes:** Only relevant with RedisJSON and RediSearch server-side support.
...
Only relevant with _RedisJSON_ and _RediSearch_ server-side support. A Redis EXPIRE timer to set/refresh on the JSON key after each JSON.MSET operation (create the JSON or add/remove subkeys), in seconds. A value of **0** disables the EXPIRE queries completely.

*Default value is 3600.*

**Notes:** Only relevant with RedisJSON and RediSearch server-side support. A value of 0 disables the EXPIRE queries completely.
...
Only relevant with _RedisJSON_ and _RediSearch_ server-side support. A key naming prefix for all internally-created Redis JSON objects (e.g. created with JSON.SET or JSON.MSET).
...
Only relevant with _RedisJSON_ and _RediSearch_ server-side support. The maximum number of results returned by each internally-triggered FT.SEARCH JSON lookup query.
...
**Notes:** Requires tls_mgm module. TLS is supported by Redis starting with version 6.0.
