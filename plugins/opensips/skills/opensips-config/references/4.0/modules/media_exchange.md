# media_exchange Module Reference
<!-- generated-from: data/4.0/modules/media_exchange.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 media_exchange module. Read this file when configuring or debugging the media_exchange module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Originates a call to the specified URI. The SDP in the response is fetched and pushed towards one of the call's legs, resulting in two way audio between the participant of the ongoing call, and the new call. By default, the other participant leg is put on hold. Can be called for an in-dialog request, such as a re-INVITE (for example when putting an entity on hold), or for an INFO request (triggered for example by a DTMF).
...
- `URI` *(string, required)* — destination used to originate the new call.

**Usable from:** ANY_ROUTE

**Example.** Use `media_exchange_from_uri()` function to fetch media from a Media Server's call.
...
- `callid` *(string, required)* — the identifier of the callid to exchange media.
- `leg` *(string, optional)* — the leg that will be streamed. Possible values are caller and callee.
  - `caller`
  - `callee`
- `nohold` *(integer, optional)* — if set to true, the other participant will not be put on hold. This is useful when a new call will be generated for the other leg as well.
...
**Example.** Use `media_exchange_to_call()` function to make an announcement.
...
Starts streaming the media of an existing proxied call, identified by the _callid_ parameter to the SDP in the request's body. Can be called multiple times, and will accept a new call for each invocation. The calls can be identified using the _instance_ parameter.

**Parameters:**

- `callid` *(string, required)* — the identifier of the callid to stream/fork media from
- `instance` *(string, optional)* — a unique name for identifying the forking instance. If missing, the default name is assumed.
- `leg` *(string, optional)* — the leg that will be streamed. Possible values are caller, callee and both. If missing, both legs will be streamed.
  ...
  - `callee`
  - `both`
- `medianum` *(integer, optional)* — the media stream that will be forked within the call. First index is 0. If missing, all media streams of that leg(s) are streamed, as long as the body has enough streams. Note: RTPProxy does not do any media mixing, therefore you need to make sure that the INVITE has enough SDP streams to handle all the media streams selected to fork.

**Usable from:** REQUEST_ROUTE, BRANCH_ROUTE, FAILURE_ROUTE, ONREPLY_ROUTE

**Example.** Use `media_fork_from_call()` function to fork all media streams of a call.
...
**Example.** Use `media_fork_from_call()` function to fork only the first caller's stream.
...
- `instance` *(string, optional)* — the forking instance to be paused. If missing, all instances are paused.
...
**Example.** Use `media_fork_pause()` function to temporarily stop the entire media stream of the call.
...
- `instance` *(string, optional)* — the forking instance to be resumed. If missing, all instances are resumed.
...
**Example.** Use `media_fork_resume()` function to resume a forking previously stopped.
...
Behaves as a B2B user agent client to initiate a call to a SIP URI and then stream the media to the SDP received in the 200 OK response. Can be called multiple times, and will create a new call for each invocation. The generated calls can be identified using the _instance_ parameter.
...
- `URI` *(string, required)* — destination where to push the current call's media

**Usable from:** ANY_ROUTE

**Example.** Use `media_fork_to_uri()` function to fork media to a Media Server.
...
**Usable from:** REQUEST_ROUTE, BRANCH_ROUTE, ONREPLY_ROUTE

**Example.** Use `media_terminate()` function to terminate an announcement.
...
Replaces obsolete MI command: _media_fork_from_call_to_uri_.
...
- `callid` *(string, required)* — the callid of the dialog that will have its RTP streamed to the new call towards the Media Server
- `headers` *(string, optional)* — extra headers to add to the outgoing request
- `instance` *(string, optional)* — the unique name of the forking instance. If missing, the _default_ name is assumed.
...
- `medianum` *(integer, optional)* — the media stream that will be forked within the call. First index is 0. If missing, all media streams of that leg(s) are streamed.
- `uri` *(string, required)* — the destination URI of the new call

**Example.** start streaming a callid to record media server
...
Replaces obsolete MI command: _media_exchange_from_call_to_uri_.
...
- `callid` *(string, required)* — the callid of the dialog that will have it's leg mixed with the new call to the Media Server
- `headers` *(string, optional)* — extra headers to add to the outgoing request
- `leg` *(string, required)* — indicates the participant that will have its media pined into the new call. Possible values are “caller” and “callee”.
...
**Example.** start playing back an annoucement to caller
...
Replaces obsolete MI command: _media_exchange_from_call_to_uri_body_.

MI command that does the same thing as the [media_exchange:from_call_to_uri](#mi_from_call_to_uri "1.4.2. media_exchange:from_call_to_uri") MI function, but also allows you to specify a custom body in the outgoing request. The body has to be specified in the mandatory _body_ parameter, all the other parameters being the same as the ones of [media_exchange:from_call_to_uri](#mi_from_call_to_uri "1.4.2. media_exchange:from_call_to_uri").

**Parameters:**

- `body` *(string, required)* — The body has to be specified in the mandatory _body_ parameter
- `callid` *(string, required)* — the callid of the dialog that will have it's leg mixed with the new call to the Media Server
- `headers` *(string, optional)* — extra headers to add to the outgoing request
- `leg` *(string, required)* — indicates the participant that will have its media pined into the new call. Possible values are “caller” and “callee”.
- `nohold` *(integer, optional)* — if set to a non-zero value, the module avoids putting the other participant on hold when the media exchanging starts
...
Replaces obsolete MI command: _media_terminate_.

MI command to terminate an ongoing media exchange.

**Parameters:**

- `callid` *(string, required)* — the callid of the dialog that will have the media exchange terminated.
- `instance` *(string, optional)* — should only be used when terminating a forking instance, and represents the instance to terminate. It must be ommitted when terminating an streaming session. However, for fallback compatibility, if the parameter is missing, and no streaming session is found, the command terminates the _default_ forking instance, if it exists.
- `leg` *(string, optional)* — the leg for whom to terminate the media exchange. Accepted values are _caller_, _callee_ and _both_. If missing, all media sessions are terminated.
...
**Example.** terminate a caller announcement
...
### Use `media_fork_to_uri()` function to fork media to a Media Server
...
...
```
### Use `media_fork_from_call()` function to fork all media streams of a call
...
...
```
### Use `media_fork_from_call()` function to fork only the first caller's stream
...
...
```
### Use `media_fork_pause()` function to temporarily stop the entire media stream of the call
...
...
```
### Use `media_fork_resume()` function to resume a forking previously stopped
...
...
```
### Use `media_exchange_from_uri()` function to fetch media from a Media Server's call
...
...
```
### Use `media_exchange_to_call()` function to make an announcement
...
...
```
### Use `media_terminate()` function to terminate an announcement
