...
<!-- generated-from: data/4.0/modules/rtp_relay.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 rtp_relay module. Read this file when configuring or debugging the rtp_relay module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The RTP session modifiers used are the ones provisioned through the [$rtp_relay](#pv_rtp_relay "1.8.1.$rtp_relay") and/or [$rtp_relay_peer](#pv_rtp_relay_peer "1.8.2.$rtp_relay_peer") variables.
...
This variable has the same meaning and parameters as the [$rtp_relay](#pv_rtp_relay "1.8.1.$rtp_relay") variable, except that it is used to provision the other UAC's flags, except the current one. All other fields are similar.
...
Replaces obsolete MI command: _rtp_relay_list_.
...
Replaces obsolete MI command: _rtp_relay_update_.
...
Replaces obsolete MI command: _rtp_relay_update_callid_.

Updates/Re-engages the RTP relays in all ongoing RTP relay sessions.

The function basically works in the same manner as [rtp_relay:update](#mi_update "1.7.2.rtp_relay:update"), but is to be used to update a specific callid. In addition, one can also update the _engine_ and _flags_ used for the particular session.
