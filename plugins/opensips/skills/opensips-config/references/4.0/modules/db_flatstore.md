# db_flatstore Module Reference
<!-- generated-from: data/4.0/modules/db_flatstore.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 db_flatstore module. Read this file when configuring or debugging the db_flatstore module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
There is a new OpenSIPS MI (management interface) command called db_flatstore:rotate. When OpenSIPS receives the command then it will close and reopen all files used by flatstore module. The rotation itself has to be done by another application (such as logrotate). Follow these steps to rotate files generated by flatstore module:
...
Replaces obsolete MI command: flat_rotate. It changes the name of the files where it is written.
...
Demonstrates setting the 'flush' parameter to 0.
...
Demonstrates setting the 'delimiter' parameter to ';'.
...
Demonstrates setting the 'suffix' parameter using a pseudo-variable.
...
Demonstrates setting the 'prefix' parameter using a pseudo-variable.
...
...
```
### Set “single_file” parameter

Demonstrates setting the 'single_file' parameter to 1.
