...
<!-- generated-from: data/4.0/modules/presence.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 presence module. Read this file when configuring or debugging the presence module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `a database module` — Required for database storage.
- `clusterer` — Required if the cluster_id module parameter is set and clustering support activated.
...
Comma Separated Value (CSV) list with the events to considered by the federated cluster - only presentities advertising one of these events will be broadcasted via the cluster. For more on presence clustering see the Section 1.2, “Presence clustering” chapter.
...
The database url. If set, the module is a fully operational presence server. Otherwise, it is used as a 'library', for its exported functions.
...
- `sender_uri` *(string, optional)* — The SIP URI. The parameter was added for enabling BLA implementation. If present, Notification of a change in published state is not sent to the respective uri even though a subscription exists. It should be taken from the Sender header. It was left at the decision of the administrator whether or not to transmit the content of this header as parameter for handle_publish, to prevent security problems.
...
Replaces obsolete MI command: pres_expose. Exposes in the script, by rasing an E_PRESENCE_EXPOSED event, all the presentities of a specific event that match a specified filter.
...
Replaces obsolete MI command: pres_phtable_list. Lists all the presentity records.
...
This event is raised for each presentity exposeed by the _presence:expose_.
