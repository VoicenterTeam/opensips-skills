# proto_udp Module Reference
<!-- generated-from: data/4.0/modules/proto_udp.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 proto_udp module. Read this file when configuring or debugging the proto_udp module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.

## Contents

- [Overview](#overview)
- [Dependencies](#dependencies)
- [Exported Parameters](#exported-parameters)
- [Configuration Examples](#configuration-examples)

## Overview

The **proto_udp** module is a built-in transport module which exports the required logic in order to handle UDP-based communication. (socket initialization and send/recv primitives to be used by higher-level network layers) Once loaded, you will be able to define _"udp:"_ listeners in your script.

## Dependencies

### OpenSIPs Modules

None.

### External Libraries

None.

## Exported Parameters

### `udp_port` (integer)

The default port to be used for all UDP related operation. Be careful as the default port impacts both the SIP listening part (if no port is defined in the UDP listeners) and the SIP sending part (if the destination URI has no explicit port).

If you want to change only the listening port for UDP, use the port option in the SIP listener defintion.

*Default value is 5060.*

**Example.** 5070.

```opensips
modparam("proto_udp", "udp_port", 5070)
```

## Configuration Examples

### Set `udp_port` parameter

The default port to be used for all UDP related operation. Be careful as the default port impacts both the SIP listening part (if no port is defined in the UDP listeners) and the SIP sending part (if the destination URI has no explicit port). If you want to change only the listening port for UDP, use the port option in the SIP listener defintion.

```opensips
...
modparam("proto_udp", "udp_port", 5070)
...
```
