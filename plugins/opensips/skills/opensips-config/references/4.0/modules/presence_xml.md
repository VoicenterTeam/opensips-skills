...
<!-- generated-from: data/4.0/modules/presence_xml.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 presence_xml module. Read this file when configuring or debugging the presence_xml module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `signaling`
- `xcap`
- `xcap_client` — Only compulsory if not using an integrated xcap server (if 'integrated_xcap_server' parameter is not set) (optional)
...
Demonstrates setting the `force_active` parameter to 1.
...
Demonstrates setting the `pidf_manipulation` parameter to 1.
...
Demonstrates setting the `xcap_server` parameter with multiple server addresses.
...
Demonstrates setting the `pres_rules_auid` parameter.
...
Demonstrates setting the `pres_rules_filename` parameter.
...
Demonstrates setting the `generate_offline_body` parameter to 0.
