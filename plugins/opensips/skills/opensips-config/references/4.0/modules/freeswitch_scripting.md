...
<!-- generated-from: data/4.0/modules/freeswitch_scripting.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 freeswitch_scripting module. Read this file when configuring or debugging the freeswitch_scripting module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Example.** :ClueCon@10.0.0.10?CHANNEL_STATE.
...
**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, ONREPLY_ROUTE, BRANCH_ROUTE, LOCAL_ROUTE, STARTUP_ROUTE, TIMER_ROUTE, EVENT_ROUTE, ERROR_ROUTE

**Example.** Example 1.9. freeswitch_esl() usage.
...
Replaces obsolete MI command: fs_list. Displays the current set of FreeSWITCH ESL sockets and the list of events that the module is subscribed to for each socket.

### `freeswitch_scripting:reload`

Replaces obsolete MI command: fs_reload. Replaces the current set* of FreeSWITCH ESL sockets along with their respective events with the current data (ESL sockets and their events) found in the "freeswitch" table. * this includes any sockets/events provisioned through fs_subscribe, MI freeswitch_scripting:subscribe commands or previous DB data set.

### `freeswitch_scripting:subscribe`

Replaces obsolete MI command: fs_subscribe. Ensures that the given FreeSWITCH ESL socket is subscribed to the given list of events. In case an event cannot be subscribed to, the freeswitch driver will periodically retry to subscribe to it until an freeswitch_scripting:unsubscribe MI command for the respective event is issued.

**Parameters:**

- `...` *(string, optional)* — (other events)
- `event` *(string, required)* — the name of the event to subscribe to
- `freeswitch_url` *(string, required)* — the ESL interface to connect to. The syntax is: [fs://][[username]:password@]host[:port][?event1[,event2]...]. The "?events" part of the URL will be silently discarded.

### `freeswitch_scripting:unsubscribe`

Replaces obsolete MI command: fs_unsubscribe. Ensures that the given FreeSWITCH ESL socket is unsubscribed from the given list of events.
