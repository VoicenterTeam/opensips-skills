# fraud_detection Module Reference
<!-- generated-from: data/4.0/modules/fraud_detection.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 fraud_detection module. Read this file when configuring or debugging the fraud_detection module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Example.** Set the `calldur_thresh_crit_col` parameter.
...
**Example.** Set the `calldur_thresh_warn_col` parameter.
...
**Example.** Set the `cpm_thresh_crit_col` parameter.
...
**Notes:** At least one db_url should be defined for the fraud_detection module to work.
...
**Notes:** Please keep in mind that a profile is merely a set of rules.
...
*Default value is fraud_detection.*
...
**Example.** Set the `totalc_thresh_crit_col` parameter.
...
**Example.** Set the `totalc_thresh_warn_col` parameter.
...
**Notes:** The default value is “0” (use local time).
...
- `number` *(string, required)* — the number the user is calling to.
- `profile_id` *(int, required)* — the fraud profile id (i.e. the subset of fraud rules) in which to try and find a matching fraud rule.
- `user` *(string, required)* — the user who is making the call. Please keep in mind that the user doesn't have to be registered. This string is only used to keep different stats for different registered users.
...
Replaces obsolete MI command: _fraud_reload_.

Reload the all the fraud rules.

### `fraud_detection:show_stats`

Replaces obsolete MI command: _show_fraud_stats_.

Show the current statistics for all dials of a _user_ to a _prefix_.

NOTE: Since the fraud statistics are refreshed on-the-fly, as check_fraud() is called, **this function will return stale data** if check_fraud() has not been called at least once for the (user, prefix) pair within a newly matching time interval!
...
- `threshold` *(string)* — the warning threshold value.
- `user` *(string)* — the user who initiated the call.
- `called_number` *(string)* — the number that was called.
- `rule_id` *(string)* — the id of the fraud rule that matched when the call was initiated
- `profile_id` *(string)* — the profile id used
### `E_FRD_WARNING`
...
Database where to load the rules from. Default value is “NULL”. At least one db_url should be defined for the fraud_detection module to work.
...
...
```
### Set the “use_utc_time” parameter

Set this parameter to non-zero in order to enable UTC-based interval matching and statistics resets, rather than local time-based. The default value is “0” (use local time).
...
...
```
### Set the “table_name” parameter

If you want to load the rules from the database you must set this parameter as the database name. The default value is “fraud_detection”.
...
...
```
### Set “rid_col” parameter

The column's name in the database storing the fraud rule's id. Default value is “ruleid”.
...
...
```
### Set “pid_col” parameter

The column's name in the database storing the fraud profile's id. Please keep in mind that a profile is merely a set of rules. Default value is “profileid”.
...
...
```
### Set “prefix_col” parameter

The column's name in the database storing the prefix for which the fraud rule will match. Default value is “prefix”.
...
...
```
### Set “start_h” parameter

The column's name in the database storing the the start time of the interval in which the rule will match. The time needs to be specified as string using the format: “HH:MM” Default value is “start_hour”.
...
The column's name in the database storing the the end time of the interval in which the rule will match. The time needs to be specified as string using the format: “HH:MM” Default value is “end_hour”.
...
...
```
### Set “cpm_thresh_warn_col” parameter

The column's name in the database storing the warning threshold value for calls per minute. Default value is “cpm_warning”.
...
...
```
### Set “cpm_thresh_crit_col” parameter

The column's name in the database storing the critical threshold value for calls per minute. Default value is “cpm_critical”.
...
...
```
### Set “calldur_thresh_warn_col” parameter

The column's name in the database storing the warning threshold value for call duration. Default value is “call_duration_warning”.
...
...
```
### Set “calldur_thresh_crit_col” parameter

The column's name in the database storing the critical threshold value for call duration. Default value is “call_duration_critical”.
...
...
```
### Set “totalc_thresh_warn_col” parameter

The column's name in the database storing the warning threshold value for the number of total calls. Default value is “total_calls_warning”.
...
...
```
### Set “totalc_thresh_crit_col” parameter

The column's name in the database storing the critical threshold value for the number of total calls. Default value is “total_calls_critical”.
...
...
```
### Set “concalls_thresh_warn_col” parameter

The column's name in the database storing the warning threshold value for the number of concurrent calls. Default value is “concurrent_calls_warning”.
...
...
```
### Set “concalls_thresh_crit_col” parameter

The column's name in the database storing the critical threshold value for the number of concurrent calls. Default value is “concurrent_calls_critical”.
...
...
```
### Set “seqcalls_thresh_warn_col” parameter

The column's name in the database storing the warning threshold value for the number of sequential calls. Default value is “sequential_calls_warning”.
...
...
```
### Set “seqcalls_thresh_crit_col” parameter

The column's name in the database storing the critical threshold value for the number of sequential calls. Default value is “sequential_calls_critical”.
