...
<!-- generated-from: data/4.0/modules/b2b_entities.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 b2b_entities module. Read this file when configuring or debugging the b2b_entities module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `extra_params` *(string, optional)* — An arbitrary value to be passed to the extra_params parameter in the E_UA_SESSION event.
- `flags` *(string, optional)* — configures options for this UA session via the following flags: t[nn] - maximum duration of this session in seconds. After this timeout a BYE will be sent and the session will be deleted. If this is not set, the default timeout, configured with ua_default_timeout will be used. Example: t3600, a - report the receving of ACK requests via the E_UA_SESSION event., r - report the receving of replies via the E_UA_SESSION event., d - disable the automatic sending of ACK upon receving a 200 OK reply for INVITE (in case of UAC session) or re-INVITE., h - provide the headers of the SIP request/reply in the E_UA_SESSION event., b - provide the body of the SIP request/reply in the E_UA_SESSION event., n - do not trigger the E_UA_SESSION event (with event_type NEW) for initial INVITES handled with this function.
  - `t[nn]`
  - `a`
  - `r`
  - `d`
...
Replaces obsolete MI command: b2be_list. This command can be used to list the internals of the b2b entities.

**Returns:** List of internals of the b2b entities.

**Example.** List the internals of the b2b entities
...
This command starts a new UAC session by sending an initial INVITE. Further requests/replies received belonging to this session will only be handled via the E_UA_SESSION event.
...
- `flags` *(string, optional)* — flags with the same meaning as for the flags paramater of ua_session_server_init().
...
**Returns:** No explicit return value documented.

**Example.** Start a new UAC session
...
List information about UA sessions started with ua_session_server_init() function or the ua_session_client_start MI function.

**Parameters:**

- `key` *(string, optional)* — b2b entity key of the UA session to list. If missing, all sessions will be listed.

**Returns:** List of information about UA sessions.

**Example.** List information about UA sessions
...
**Example.** Send a reply for a UA session
...
**Example.** Terminate a UA session
...
**Example.** Send a sequential request for a UA session
...
Note that replies will not be reported at all unless the r flag was set when initiating the UA session. Also ACK requests are only reported if the a flag was set.

**Parameters:**

- `key` *(string)* — b2b entity key of the UA session.
- `entity_type` *(string)* — indicates whether this is a UAS or UAc entity.
...
- `extra_params` *(string)* — an arbitrary value. Currently only the ua_session_server_init() function passes this if the extra_params argument is used, and it only appears in the NEW event_type.

## Configuration Examples

### Set `server_hsize` parameter

Sets the size of the hash table that stores the b2b server entities.
...
Sets the size of the hash table that stores the b2b client entities.
...
Sets the name of the b2b script route that will be called when B2B requests are received.
...
Sets the name of the b2b script route that will be called when B2B replies are received.
...
Sets the URL of a NoSQL database to be used.
...
Sets the prefix to use for every key set in the NoSQL database.
...
Sets the time interval at which to update the info in database.
...
Sets the string to use when generating the key.
...
Sets the B2B modules database storage mode.
...
Sets the name of the table that will be used for storing B2B entities.
...
Sets the ID of the cluster this instance belongs to.
...
Controls whether a PRACK should be generated locally or end-to-end.
...
Sets the contact to use in generated messages for UA session.
...
Sets the default timeout, in seconds, for UA session.
...
Demonstrates how to initialize a new UA session by processing an initial INVITE.
...
Demonstrates how to send a sequential request for a UA session.
...
Demonstrates how to send a reply for a UA session.
...
Demonstrates how to terminate a UA session.
...
Shows the structure of the b2b_api_t.
