...
<!-- generated-from: data/4.0/modules/topology_hiding.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 topology_hiding module. Read this file when configuring or debugging the topology_hiding module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
```
### `th_callid_loop_protection` (integer)

Include the _from_tag_ when encoding the topology-hiding Call-ID to ensure correct decoding in looping scenarios, (when the same call with a previously encoded Call-ID is being looped back).

Note that enabling this parameter will increase the generated Call-ID value, due to the additional from_tag information being embedded.
...
### `topology_hiding([flags], [contact_user])`
...
- `true` — If the request is successfully processed
...
**Example.** Set "opensips" for both caller and the callee's Contact username.
...
**Example.** Set "caller" in the caller's Contact username.
...
**Example.** Set "callee" in the callee's Contact username.
...
**Example.** Set "caller" in the caller's Contact username and "callee" in the callee's Contact username.
...
- `dlg_match_mode` *(string, optional)* — Optional parameter for dialog matching. Behaves identically to match_dialog(). Please see the dialog module documentation for further details regarding dialog matching options.
...
...
```
### Set `th_callid_loop_protection` parameter
