...
<!-- generated-from: data/4.0/modules/json.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 json module. Read this file when configuring or debugging the json module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `$json(dest_id)` *(json, required)* — Destination json object
- `$json(source_id)` *(json, required)* — Source json object
...
- `main_json_var` *(string, required)* — Main json variable
- `output_var` *(string, required)* — Output variable
- `patch_json_var` *(string, required)* — Patch json variable
...
The `json_compact` variable has the same purpose as the [$json](#pv_json "1.4.1.$json(id)") variable, but prints the JSON object in a more compact form, without formatting spaces.
...
The `json_compact_noescape` variable has the same purpose as the [$json_compact](#pv_json_compact "1.4.3.$json_compact(id)") variable, printing the JSON object in the compact form, but without escaping the slashes.

_NOTICE:_ due to the libjson-c library limitations, this variable only skips the slashes escaping starting with version _0.13_ - older versions of the library make the variable behave just like the [$json_compact](#pv_json_compact "1.4.3.$json_compact(id)") variable.

**Example1.13.Difference between json_compact and json_compact_noescape**
...
The `json_pretty` variable has the same purpose as the [$json](#pv_json "1.4.1.$json(id)") variable, but prints the JSON object in a pretty format, adding spaces and tabs to make the output more readable.
