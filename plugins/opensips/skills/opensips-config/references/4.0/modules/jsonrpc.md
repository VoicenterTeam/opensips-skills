...
<!-- generated-from: data/4.0/modules/jsonrpc.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 jsonrpc module. Read this file when configuring or debugging the jsonrpc module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The amount of milliseconds OpenSIPS waits for the JSON-RPC server to respond to a JSON-RPC request, until it times out. Note that these parameter only affects the _jsonrpc_request_ command.
...
Sets the `write_timeout` parameter.
...
Sets the `read_timeout` parameter.
...
Demonstrates usage of the `jsonrpc_request()` function.
...
Demonstrates usage of the `jsonrpc_notification()` function.
