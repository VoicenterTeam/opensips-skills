...
<!-- generated-from: data/4.0/modules/permissions.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 permissions module. Read this file when configuring or debugging the permissions module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Function for registration checking is called `allow_register` and the algorithm is very similar to the algorithm described in [Section1.1.1, “Call Routing”](#sec-call-routing "1.1.1.Call Routing"). The only difference is in the way how pairs are created.
...
The algorithm of matching is same as described in [Section1.1.1, “Call Routing”](#sec-call-routing "1.1.1.Call Routing").
...
**Example.** Allow routing using specific basename.
...
**Example.** Various usage examples of check_address.
...
- `context_info` *(var, optional)* — Variable to store the context_info field from the cached address table.
- `group_id` *(int, required)* — Group id to be matched.
- `partition` *(string, optional)* — Optional partition name.
- `pattern` *(string, optional)* — String to be matched against the wildcard pattern field.
...
**Example.** Check source address with context info and partition.
...
- `partition` *(string, optional)* — Optional partition name. If no partition specified, the “default” one will be used.
...
**Example.** Get source group and log it.
