...
<!-- generated-from: data/4.0/modules/enum.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 enum module. Read this file when configuring or debugging the enum module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Notes:** If set to "cc", i_enum_query() will always inserts the label at the country-code level. Examples: i.1.e164.arpa, i.3.4.e164.arpa, i.2.5.3.e164.arpa
...
**Example.** Set the `bl_algorithm` parameter.
...
**Example.** Set the `branchlabel` parameter.
...
**Example.** Set the `domain_suffix` parameter.
...
**Example.** Set the `i_enum_suffix` parameter.
...
**Example.** Set the `isn_suffix` parameter.
...
**Example.** Set the `tel_uri_params` parameter.
...
The domain suffix to be used for isn_query() lookups.
...
The function performs an ENUM query on a given E.164 "number" (or R-URI username if "number" is missing) and rewrites the Request-URI with the result of the query.
...
The function performs a ISN query and rewrites the Request-URI with the result of the query.
