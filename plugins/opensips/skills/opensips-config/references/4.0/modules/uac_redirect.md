...
<!-- generated-from: data/4.0/modules/uac_redirect.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 uac_redirect module. Read this file when configuring or debugging the uac_redirect module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The regular expression for default accept filtering. It make sens to be defined on only if the default_filter parameter is set to “deny”. All contacts matching the accept_filter will be accepted; the rest of them will be rejected for redirection. The parameter may be defined only one - multiple definition will overwrite the previous definitions. If more regular expression need to be defined, use the set_accept_filter() scripting function.

*Default value is NULL.*

**Example.** Set the `accept_filter` parameter.
...
**Example.** Set the `default_filter` parameter.
