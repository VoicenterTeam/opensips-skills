...
<!-- generated-from: data/4.0/modules/msrp_ua.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 msrp_ua module. Read this file when configuring or debugging the msrp_ua module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
In order to start a SIP call carying MSRP from OpenSIPS you can use the [msrp_ua:start_session](#mi_start_session "1.6.2. msrp_ua:start_session") MI function. Alternatively, to answer a SIP session with MSRP you can use the [msrp_ua_answer()](#func_msrp_ua_answer "1.5.1. msrp_ua_answer(content_types)") script function.
...
You can send MSRP messages to the peer with the [msrp_ua:send_message](#mi_send_message "1.6.1. msrp_ua:send_message") MI function.

## Dependencies

### OpenSIPs Modules

- `b2b_entities` — The following modules must be loaded before this module
- `proto_msrp` — The following modules must be loaded before this module
...
Contact to be used in the generated SIP requests. For sessions answered by OpenSIPS, if it is not set, it is constructed dynamically from the socket where the initiating request was received. This parameter is mandatory when using the msrp_ua:start_session MI function.
...
- `content_types` *(string, required)* — content types adevertised in the _accept-types_ SDP attribute. At least one of the content types in this list must match the types offered by the peer in its SDP offer.
...
Replaces obsolete MI command: _msrp_ua_end_session_.
...
Replaces obsolete MI command: _msrp_ua_list_sessions_.
...
Replaces obsolete MI command: _msrp_ua_send_message_.
...
- `session_id` *(string, required)* — the MSRP session identifier ("session-id" part of the MSRP URI).
- `success_report` *(string, optional)* — string indicating whether to request an MSRP Success Report. Possible values are _yes_ or _no_. If the parameter is missing or is set to "no" the SEND request will not include a Success-Report header.
...
Replaces obsolete MI command: _msrp_ua_start_session_.

Starts a MSRP session.

The [advertised_contact](#param_advertised_contact "1.4.5.advertised_contact (string)") is mandatory if this function is used.

**Parameters:**

- `content_types` *(string, required)* — content types adevertised in the _accept-types_ SDP attribute.
...
This event is triggered when an ongoing MSRP session is terminted (session expires or BYE is received; terminating a session via the _msrp_ua:end_session_ MI function is not included).
