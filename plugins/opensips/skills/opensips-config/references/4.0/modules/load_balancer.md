...
<!-- generated-from: data/4.0/modules/load_balancer.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 load_balancer module. Read this file when configuring or debugging the load_balancer module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `true` — if the call was properly taken into consideration for estimating the load on the destination
...
- `true` — if found and active (see the "active" parameter)
...
- `true` — if there is an ongoing LB session
...
- `grp` *(int, required)* — group id for the destinations
- `resources` *(string, required)* — a semi-colon separated list of resources required by the current call
...
Old name of the lb_start_or_next() function. Take care, this will become obsolete.
...
Replaces obsolete MI command: _lb_list_. Lists all the destinations and the maximum and current load for each resource of the destination.

**Example.** load_balancer:list usage
...
Replaces obsolete MI command: _lb_reload_. Trigers the reload of the load balancing data from the DB.
...
Replaces obsolete MI command: _lb_resize_. Changes the capacity for a resource of a destination.
...
Replaces obsolete MI command: _lb_status_. Gets or sets the status (enabled or disabled) of a destination.
...
**Example.** load_balancer:status usage
