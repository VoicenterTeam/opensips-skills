# rr Module Reference
<!-- generated-from: data/4.0/modules/rr.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 rr module. Read this file when configuring or debugging the rr module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The function checks the flow direction of the request. As for checking it's used the “ftag” Route header parameter, the append_fromtag (see append_fromtag module parameter must be enabled. Also this must be called only after loose_route() (see loose_route()).
...
- `FALSE` — If there is no Route header, if there is only one Route header indicating the local proxy, or if there is a Route header but other parsing errors occur
- `TRUE` — If a Route header is found and behaves as described in section 16.12 of RFC 3261
...
* string2 (optional) - String to be inserted into the second header field.
...
- `string2` *(string, optional)* — String to be inserted into the second header field.
