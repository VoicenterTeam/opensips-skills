...
<!-- generated-from: data/4.0/modules/rate_cacher.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 rate_cacher module. Read this file when configuring or debugging the rate_cacher module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `client_id` *(string/integer/pvar, required)* — The pseudo-var will hold the client_id originating this call
- `desired_margin` *(string/integer/pvar, required)* — The pseudo-var contains the minimum Integer margin that the script writer wants to achieve, based on the Client sell and Vendor buy prices. The formula used is : vendor_margin=(client_price - results[i])*100/client_price) . If the vendor_margin is higher than the desired_margin, then the Vendor is ok to use. The desired margin can be positive ( call will be profitable ) or negative ( the call will cause a loss ).
- `dialled_no` *(string/integer/pvar, required)* — The pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `is_wholesale` *(string/integer/pvar, required)* — The pseudo-var will contain either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning )
  - `1`
  - `0`
- `out_vendor_csv` *(string/integer/pvar, required)* — The pseudo-var is an output parameter, and the pvar will get populated with the CSV list of Vendors that meet the desired margin condition
- `vendors_csv` *(string/integer/pvar, required)* — The pseudo-var contains a list of Vendors that need to be filtered based on the desired margin ( keep just those that match your desired percentage margin for this call )
...
**Example.** Example 1.11. cost_based_filtering usage.
...
**Example.** Example 1.12. cost_based_ordering usage.
...
- `client_id` *(string/integer/pvar, required)* — The pseudo-var will hold the client_id originating this call
- `destination_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched destination from the client's ratesheet
- `dialled_no` *(string/integer/pvar, required)* — The pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched increment from the client's ratesheet
- `is_wholesale` *(string/integer/pvar, required)* — The pseudo-var will contain either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning )
  - `1`
  - `0`
- `minimum_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched minimum from the client's ratesheet
- `prefix_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched prefix from the client's ratesheet
- `price_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched price from the client's ratesheet
...
**Example.** Example 1.9. get_client_price usage.
...
- `destination_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched destination from the vendor's ratesheet
- `dialled_no` *(string/integer/pvar, required)* — The pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched increment from the vendor's ratesheet
- `minimum_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched minimum from the vendor's ratesheet
- `prefix_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched prefix from the vendor's ratesheet
- `price_pvar` *(string/integer/pvar, required)* — The pseudo-var will contain the matched price from the vendor's ratesheet
- `vendor_id` *(string/integer/pvar, required)* — The pseudo-var will hold the vendor_id
...
**Example.** Example 1.10. get_vendor_price usage.
...
Replaces obsolete MI command: rc_addClient. Adds a new Client, without assigning any ratesheet to it.
...
Replaces obsolete MI command: rc_addVendor. Adds a new Vendor, without assigning any ratesheet to it.
...
Replaces obsolete MI command: rc_deleteClient. Removes a Client from memory, along with the ratesheet asigned with it ( if any )
...
Replaces obsolete MI command: rc_deleteClientRate. Deletes the assigned ratesheet from the Client
...
**Example.** Deletes a Client Ratesheet
...
Replaces obsolete MI command: rc_deleteVendor. Removes a vendor from memory, along with the ratesheet asigned with it ( if any )
...
Replaces obsolete MI command: rc_deleteVendorRate. Deletes the assigned ratesheet from the Vendor
...
Replaces obsolete MI command: rc_getClientPrice. Fetches all the ratesheet information ( destination name, price, minimum, increment ) for the provided Client, on the specified quality ( wholesale vs retail ) and dialled number
...
**Returns:** JSON object containing ratesheet information (structured response — see schema)
...
Replaces obsolete MI command: rc_getVendorPrice. Fetches all the ratesheet information ( destination name, price, minimum, increment ) for the provided Vendor and dialled number
...
Replaces obsolete MI command: rc_reloadClientRate. Reloads the provided ratesheet and assigns it to the Client
...
Replaces obsolete MI command: rc_reloadVendorRate. Reloads the provided ratesheet and assigns it to the Vendor
