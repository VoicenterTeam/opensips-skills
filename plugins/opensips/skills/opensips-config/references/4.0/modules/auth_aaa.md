...
<!-- generated-from: data/4.0/modules/auth_aaa.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 auth_aaa module. Read this file when configuring or debugging the auth_aaa module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
*   _value = SIP_AVP_NAME SIP_AVP_VALUE_
    
*   _SIP_AVP_NAME = STRING_NAME | '#'ID_NUMBER_
    
*   _SIP_AVP_VALUE = ':'STRING_VALUE | '#'NUMBER_VALUE_
...
- `aaa_radius` — AAA protocol implementation (example of an aaa implementing module)
...
- `sip_uri` *(string, optional)* — SIP URI stored in the "sip_uri" parameter (or user@host part of the Request-URI if "sip_uri" is not given).
...
- `sip_uri` *(string, optional)* — user stored in "sip_uri" (or Request-URI user part if "sip_uri" is not given).
...
The function verifies credentials according to RFC2617. If the credentials are verified successfully then the function will succeed and mark the credentials as authorized (marked credentials can be later used by some other functions). If the function was unable to verify the credentials for some reason then it will fail and the script should call `proxy_challenge` which will challenge the user again. This function will, in fact, perform sanity checks over the received credentials and then pass them along to the aaa server which will verify the credentials and return whether they are valid or not.
...
The function verifies credentials according to RFC2617. If the credentials are verified successfully then the function will succeed and mark the credentials as authorized (marked credentials can be later used by some other functions). If the function was unable to verify the credentials for some reason then it will fail and the script should call `www_challenge` which will challenge the user again. This function will, in fact, perform sanity checks over the received credentials and then pass them along to the aaa server which will verify the credentials and return whether they are valid or not.
...
### “SIP-AVP” AAA AVP examples

Demonstrates the syntax of SIP-AVP values used for additional credentials.
...
Demonstrates setting the `aaa_url` parameter.
...
Demonstrates setting the `auth_service_type` parameter.
...
Demonstrates setting the `check_service_type` parameter.
...
Demonstrates setting the `use_ruri_flag` parameter.
...
Demonstrates usage of the `aaa_www_authorize` function.
...
Demonstrates usage of the `proxy_authorize` function.
...
Demonstrates usage of the `aaa_does_uri_exist` function.
...
Demonstrates usage of the `aaa_does_uri_user_exist` function.
