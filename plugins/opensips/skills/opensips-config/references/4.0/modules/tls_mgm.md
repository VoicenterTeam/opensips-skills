...
<!-- generated-from: data/4.0/modules/tls_mgm.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 tls_mgm module. Read this file when configuring or debugging the tls_mgm module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `tls_openssl or tls_wolfssl` — Must be loaded before this module, unless tls_library is set to 'none'
...
**Example.** Set the `ca_dir` parameter.
...
**Example.** Set the `ca_list` parameter.
...
**Example.** Set the `ciphers_list` parameter.
...
**Notes:** Note: If there is already an existing TLS connection to the remote target, it will be reused and setting this AVP has no effect. Note: You can force a particular SIP domain to be used just for a particular branch by setting the _$bavp_ variable with the same name. When both _$bavp_ and _$avp_ variables are set, the first one takes precedence.
...
**Example.** Set the `crl_check_all` parameter.
...
**Example.** Set the `crl_dir` parameter.
...
**Notes:** If you are using an OpenSSL library newer than 1.1.0, you can also specify a range of accepted TLS versions as [VLOW]-[VHIGH].
...
Replaces obsolete MI command: _tls_list_.

List all domains information.

### `tls_mgm:reload`

Replaces obsolete MI command: _tls_reload_.
...
Public certificate file for OpenSIPS.
...
Directory storing certificate revocation lists (CRLs).
...
You can specify a file which contains Diffie-Hellman parameters as a PEM-file.
...
Activates SSL_VERIFY_PEER in the ssl_context.
...
Name of the AVP used for enforcing the selection of a specific TLS client domain.
...
The IP addresses and ports used to match a TLS connection with a virtual TLS domain.
...
The SIP domains used to match a TLS connection with a virtual TLS domain.
