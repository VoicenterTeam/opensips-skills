...
<!-- generated-from: data/4.0/modules/aaa_diameter.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 aaa_diameter module. Read this file when configuring or debugging the aaa_diameter module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `libfdcore` — Diameter message building, parsing, peer state machine and network communication (v1.2.1 or higher)
- `libfdproto` — Diameter message building, parsing, peer state machine and network communication (v1.2.1 or higher)
...
**Example.** "diameter:freeDiameter-client.conf".
...
When an error log is printed due to malformed JSON, this parameter indicates how many characters from the JSON should be printed at console. A higher value might overcrowd the logs, but can be useful for troubleshooting.
...
- `rpl_avps_pv` *(var, optional)* — output variable which will hold all AVP names from the Diameter Answer along with their values, packed as a JSON Array string. The "json" module and its _$json_ variable could be used to iterate this array.
...
This event is raised whenever the _aaa_diameter_ module is loaded and OpenSIPS receives a Diameter request on the configured Diameter listening interface.

**Parameters:**

- `app_id` *(integer)* — the Diameter Application Identifier
- `cmd_code` *(integer)* — the Diameter Command Code
- `sess_id` *(string)* — the value of either the _Session-Id_ AVP, _Transaction-Id_ AVP or a _NULL_ value if neither of these transaction-identifying AVPs is present in the Diameter request.
...
**Subscribe via:** Mainly consumed by an event_route, but can be consumed through any other EVI-compatible delivery channel.
...
modparam("aaa_diameter", "answer_timeout", 5000)
```
### Setting the `max_json_log_size` parameter
