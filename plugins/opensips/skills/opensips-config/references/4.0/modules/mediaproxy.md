# mediaproxy Module Reference
<!-- generated-from: data/4.0/modules/mediaproxy.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 mediaproxy module. Read this file when configuring or debugging the mediaproxy module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `dialog` — Required if engage_media_proxy is used

### External Libraries

- `MediaProxy` — External application (version 2.0.0 or newer) required by this module
...
**Example.** /run/mediaproxy/dispatcher.sock.
...
...
```
### Setting the `mediaproxy_socket` parameter
...
Specification of the AVP which holds the IP address from where the SIP signaling originated.
...
Specification of the AVP which holds an optional application defined media relay IP address of a particular media relay that is preferred to be used for the current call.
...
Specification of the AVP which holds the ICE candidate that will be inserted in the SDP.
...
Trigger the use of MediaProxy for all the dialog requests and replies that have an SDP body.
...
...
```
### Using the `use_media_proxy` function

Will make a call to the dispatcher and replace the IPs and ports in the SDP body with the ones returned by the media relay for each supported media stream in the SDP body.
...
...
```
### Using the `end_media_session` function

Will call on the dispatcher to inform the media relay to end the media session.
