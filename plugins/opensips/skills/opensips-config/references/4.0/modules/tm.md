# tm Module Reference
<!-- generated-from: data/4.0/modules/tm.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 tm module. Read this file when configuring or debugging the tm module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
For how to define branch flags and use via script, see [t_on_branch()](#func_t_on_branch "1.4.13. t_on_branch(branch_route)") and the setbflag(), resetbflag() and isbflagset() script functions.
...
*   **[fr_timeout](#param_fr_timeout "1.3.1.fr_timeout (integer)")** - used when no response was received yet. If there is no response after _[fr_timeout](#param_fr_timeout "1.3.1.fr_timeout (integer)")_ seconds, the timer triggers (and failure route will be executed if t_on_failure() was called). For INVITE transactions, if a provisional response was received, the timeout is reset to _[fr_inv_timeout](#param_fr_inv_timeout "1.3.2.fr_inv_timeout (integer)")_ seconds and RT_T2 for all other transactions. Once a final response is received, the transaction has finished.
    
*   fr_inv_timeout - this timeout starts counting down once a provisional response was received for an INVITE transaction.

For example: You want to have failover if there is no provisional response after 3 seconds, but you want to ring for 60 seconds. Thus, set the _[fr_timeout](#param_fr_timeout "1.3.1.fr_timeout (integer)")_ to 3 and fr_inv_timeout to 60.
...
- `re` *(string, required)* — Regular expression to match the reply code of the response message.
...
- `flags` *(string, optional)* — some additional flags related to the injection process: "cancel" or "c" - cancel all the ongoing existing branches from the transaction before injecting the new branches; "l" (last) - this is the last injected branch on this transaction, do not wait for any other branches to be injected.
  - `cancel`
  - `c`
  - `l`
- `source` *(string, required)* — where to take the description for the new branches to be injected. It can be "event" - the branch will be taken from the event attributes exposed in an event notification route (see event_routing module) or "msg" - the branches will be taken from the RURI of the SIP message and from the additional branches (created by append_branch() function or similar).
  - `event`
  - `msg`

**Usable from:** timer route, event route, notification route, other
...
- `reply` *(string, required)* — Parameter may be “all” or “last”.
  - `all`
  - `last`

**Return codes:**

- `true` — if all or last (depending of the parameter) reply(es) were local generated (and not received)

**Usable from:** REQUEST_ROUTE, BRANCH_ROUTE, FAILURE_ROUTE, ONREPLY_ROUTE

**Example.** t_local_replied usage.
...
- `flags` *(string, optional)* — A comma separated list of string flags for controlling the internal behaviour.
  ...
  - `pass-reason-hdr`
  - `allow-no-cancel`
- `outbound_proxy` *(string, optional)* — A string representing an outbound proxy (a fixed destination) where the message should be sent. The destination is specified as “[proto:]host[:port]”. If a destination URI “$du” for this message was set before the function is called then this value will be used as the destination instead of the function parameter.
...
- `uri` *(string, required)* — The destination is specified by a SIP URI.
...
- `callid` *(string, optional)* — Call-ID used to identify the transaction.
- `code` *(int, required)* — Reply code number.
- `cseq` *(int, optional)* — CSEQ used to identify the transaction.
- `reason_phrase` *(string, required)* — Reason string.
...
- `info` *(string, required)* — Information identifier for tw_append.
- `sock` *(string, required)* — Path to the UNIX socket.
...
similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but gives read/write access to the attributed attached to the branch.

An attribute can have whatever name (no need to be pre-defined) and it can have a single value (at a time), string or integer.

- **Type:** string
- **Read/write:** read-write
- **Scope:** BRANCH, ONREPLY, FAILURE
### `$tm.branch.duri[]`

100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the Detination-URI value of the branch.
...
similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but gives read/write access to a single branch flag (by its name).
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the list (comma separated) of per-branch flags which are set for the branch.
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the reply code of the last received reply (from the network) on this branch. NULL is returned in no reply was received so far.
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the PATH value of the branch.
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the Q value of the branch.
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the socket description (proto:ip:port) used for sending the branch out.
...
100% similar to **[$tm.branch.uri](#pv_tm_branch_uri "1.5.9.$tm.branch.uri[]")**, but returning the type of the current branch. This may be "phone" if it not a real branch (has no SIP signalling, used by waiting for branch injection) or "sip" (a real signalling branch).
...
gives read-only access over the Request URI (as string) of a TM existing branch. The status of the branch (completed, ongoing, etc) is not relevant.

The TM (UAC side) branches are created when the request is sent to new destinations via "t_relay()" or "t_inject()".
...
* _/active_ - the indexing starts also from 0, but it is relative to the last set of branches - the parallel branches created by the last "t_relay()"-ing.
* _/all_ - similar to "no suffix" case, meaning it is an absolute index, covering all the branches of the trasactions (resulted from all "t_relay()"s performed over the transaction).
...
Replaces obsolete MI command: _t_hash_.
...
Replaces obsolete MI command: _t_reply_.
...
Replaces obsolete MI command: _t_uac_cancel_.
...
Replaces obsolete MI command: _t_uac_dlg_.
...
Sets the fr_timeout parameter to 10 seconds.
...
Sets the fr_inv_timeout parameter to 200 seconds.
...
Sets the wt_timer parameter to 10 seconds.
...
Sets the delete_timer parameter to 5 seconds.
...
Sets the T1_timer parameter to 700 milliseconds.
...
Sets the T2_timer parameter to 8000 milliseconds.
...
Sets the ruri_matching parameter to 0 (false).
...
Sets the via1_matching parameter to 0 (false).
...
Sets the unix_tx_timeout parameter to 5 seconds.
...
Sets the restart_fr_on_each_reply parameter to 0 (false).
...
Sets the tw_append parameter to append User-Agent, AVP, message body, and timestamp.
...
Sets the pass_provisional_replies parameter to 1 (enabled).
...
Sets the syn_branch parameter to 0 (disabled).
...
Sets the onreply_avp_mode parameter to 1.
...
Sets the disable_6xx_block parameter to 1.
...
Sets the minor_branch_flag parameter to "MINOR_BFLAG".
...
Sets the timer_partitions parameter to 2.
...
Sets the auto_100trying parameter to 0 (disabled).
...
Sets the tm_replication_cluster parameter to 1.
...
Sets the cluster_param parameter to "tid".
...
Sets the cluster_auto_cancel parameter to "no".
...
Sets the local_request_route parameter to "tm_local_request".
...
Sets the local_reply_route parameter to "tm_local_reply".
...
Demonstrates how to relay a message statefully, including error handling and specifying outbound proxies.
...
Demonstrates how to send a stateful SIP reply.
...
Demonstrates how to send a stateful SIP reply with a body.
...
Demonstrates how to create a SIP transaction.
...
Demonstrates how to check if a request is associated with a transaction.
...
Demonstrates how to check the reply code of a response message.
...
Demonstrates how to check if replies were locally generated.
...
Demonstrates how to check if a transaction was cancelled by the UAC.
...
Demonstrates how to cancel branches upon receiving a specific reply.
...
Demonstrates how to generate and send a new SIP request.
...
Demonstrates how to set a failure route.
...
Demonstrates how to set reply routes for transactions and specific branches.
...
Demonstrates how to set a branch route.
...
Demonstrates how to inject branches from an event notification.
...
Demonstrates how to make a transaction wait for new branches.
...
Demonstrates how to stop waiting for new branches.
...
Demonstrates how to attach headers to a transaction.
...
Demonstrates how to add a custom Reason header to a CANCEL request.
...
Demonstrates how to replicate a request to another destination.
...
Demonstrates how to write request information via FIFO or UNIX socket.
...
Demonstrates how to flush flags from the current request into the transaction.
...
Demonstrates how to replicate CANCEL or ACK messages in an anycast setup.
...
Demonstrates how to send a reply to an existing INVITE transaction identified by Call-ID.
...
Demonstrates how to search for a branch index by attribute.
