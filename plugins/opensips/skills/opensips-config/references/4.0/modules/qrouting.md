...
<!-- generated-from: data/4.0/modules/qrouting.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 qrouting module. Read this file when configuring or debugging the qrouting module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `SQL DB module` — offering access to the "qr_profiles" table
- `dialog`
- `drouting`
- `tm`
...
*   **"dynamic-weights"** - for each prefix, all destinations start with equal weights and receive an equal share of the traffic. As signaling statistics are gathered for the destinations, the ones which underperform will receive less traffic, based on the "penalty" columns of the _qr_profiles_ table
...
**Notes:** for optimal results when using the "best-dest-first" algorithm, the destinations must be provisioned in descending order of their expected quality! (i.e. best quality gateways must be placed towards the start of the list)
...
*   warn_threshold__<STAT>
*   crit_threshold__<STAT>
*   warn_penalty__<STAT>
*   crit_penalty__<STAT>
...
Within a given routing rule, temporarily remove the given gateway or carrier from routing, until they are re-enabled via qr_enable_dst() or qrouting:enable_dst. The removal effect will be lost on an OpenSIPS restart.
...
Replaces obsolete MI command: _qr_disable_dst_.
...
Replaces obsolete MI command: _qr_enable_dst_.
...
Replaces obsolete MI command: _qr_reload_.
...
Replaces obsolete MI command: _qr_status_.

Inspect the signaling quality statistics of the current [history_span](#param_history_span "1.4.4.history_span (integer)") for all drouting gateways in all partitions, with various levels of filtering.
