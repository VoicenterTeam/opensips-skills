...
<!-- generated-from: data/4.0/modules/snmpstats.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 snmpstats module. Read this file when configuring or debugging the snmpstats module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `Net SNMP DEV` — SNMP library (development files) must be installed at the time of compilation. Furthermore, there are several shared objects that must be loadable at the time SNMPStats is loaded. This means that SNMP lib must be installed (but not necessarily running) on the system that has loaded the SNMPStats module.
- `SNMP tools` — SNMP tools package to provide the snmpget command (internally used by the SNMPStats module.
...
Triggered when the number of active dialogs has passed a major threshold.

**Subscribe via:** Configure the SNMP daemon (snmpd.conf) to receive traps using `trap2sink`.
...
Triggered when the number of active dialogs has passed a minor threshold.
...
Triggered when the number of bytes waiting to be consumed across all of OpenSIPS's listening ports has passed a major threshold.
...
Triggered when the number of bytes waiting to be consumed across all of OpenSIPS's listening ports has passed a minor threshold.
