...
<!-- generated-from: data/4.0/modules/auth_jwt.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 auth_jwt module. Read this file when configuring or debugging the auth_jwt module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `libjwt-dev` — Required for running OpenSIPS with this module
- `libssl-dev` — Required for running OpenSIPS with this module
- `openssl-dev` — Required for running OpenSIPS with this module
...
**Notes:** Parameter syntax:

*   _load_credentials = credential (';' credential)*_
...
- `e` *(string, required)* — Base64url-encoded RSA exponent. The string may contain pseudo variables.
- `n` *(string, required)* — Base64url-encoded RSA modulus. The string may contain pseudo variables.
- `out_public_key` *(pvar, required)* — PVAR used to store the extracted public key
...
Demonstrates setting the `db_mode` parameter.
...
Demonstrates setting the `profiles_table` parameter.
...
Demonstrates setting the `secrets_table` parameter.
...
Demonstrates setting the `tag_column` parameter.
...
Demonstrates setting the `username_column` parameter.
...
Demonstrates setting the `secret_tag_column` parameter.
...
Demonstrates setting the `secret_column` parameter.
...
Demonstrates setting the `start_ts` parameter.
...
Demonstrates setting the `end_ts` parameter.
...
Demonstrates setting the `tag_claim` parameter.
...
Demonstrates setting the `load_credentials` parameter.
...
Demonstrates using the `jwt_db_authorize` function.
...
Demonstrates using the `jwt_script_authorize` function.
...
Demonstrates using the `extract_pub_key_from_cert` function.
...
...
```
### `extract_pub_key_from_exp_mod` usage

Demonstrates using the `extract_pub_key_from_exp_mod` function.
