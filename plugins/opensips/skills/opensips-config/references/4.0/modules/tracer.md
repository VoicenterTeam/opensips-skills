...
<!-- generated-from: data/4.0/modules/tracer.json
     generator-version: 0.1.0
     opensips-version: 4.0
     doc-type: module -->

Reference for the OpenSIPs 4.0 tracer module. Read this file when configuring or debugging the tracer module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `dialog` — only if you want to trace SIP dialogs (INVITE based)
- `proto_hep` — only if you want to trace / replicate messages over HEP protocol
...
```opensips
modparam("tracer", "syslog_default_level", 2)
```
### `trace_id` (string)
...
When the uri is a file, the path to the file has to be specified after the colon. The output is always appended if the file exists, or created if it doesn't, using file_mode permissions.

When the uri is syslog, it has to follow the following format: syslog[:FACILITY[:LEVEL]]. The default facility and levels are the ones used by OpenSIPS (syslog_facility and log_level). These can be tuned using syslog_default_facility and syslog_default_level parameters.
...
**Example.** Example 1: how to trace a dialog sip and xlog.
...
**Example.** Example 2: how to trace initial INVITE and BYE, sip and rest.
...
**Example.** Example 3: trace initial INVITE transaction's only xlog and rest, no sip.
...
**Example.** Example 4: stateless transaction aware mode!.
...
**Returns:** Returns the global tracing state and dumps info about trace ids.
...
Replaces obsolete MI command: _trace_start_. Creates a dynamic tracing destination based using custom filters. This function can be used to debug calls for certain destinations real-time. Dynamic destinations are not restart persistent!
...
**Returns:** Confirmation of the dynamic tracing destination creation.
...
Replaces obsolete MI command: _trace_stop_. Stops OpenSIPS from sending traffic to a dynamic trace id created using the _trace_start_ command.

**Parameters:**

- `id` *(string, required)* — the name of the tracing instance to be stopped.

**Returns:** Confirmation of stopping the trace.
