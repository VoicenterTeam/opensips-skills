...
Generated reference for OpenSIPs 3.4. This file is the module catalog for the `opensips-config` skill. It maps every module in the active reference set to its per-module reference file and provides the lookup-discipline guidance that governs how Claude uses the reference set.
## Module index
...
| `aaa_diameter` | This module provides a Diameter implementation for the core AAA API interface | `references/{version}/modules/aaa_diameter.md` |
...
| `avpops` | AVPops (AVP-operations) modules implements a set of script functions which allo… | `references/{version}/modules/avpops.md` |
...
| `clusterer` | The clusterer module is used to organize multiple OpenSIPS instances into group… | `references/{version}/modules/clusterer.md` |
...
| `proto_msrp` | The proto_msrp module provides the MSRP protocol stack, meaning the network rea… | `references/{version}/modules/proto_msrp.md` |
...
| `proto_tcp` | The proto_tcp module is a built-in transport module which implements SIP TCP-ba… | `references/{version}/modules/proto_tcp.md` |
...
| `script_helper` | The purpose of the **Script Helper module** is to simplify the scripting proces… | `references/{version}/modules/script_helper.md` |
