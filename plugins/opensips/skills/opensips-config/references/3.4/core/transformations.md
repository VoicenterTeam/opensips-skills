...
<!-- generated-from: data/3.4/core/transformations.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: transformation -->

Reference for OpenSIPs 3.4 transformations. Read this file when manipulating pseudo-variable values inline and need the canonical class, input/output, and chaining behavior of a transformation.
...
- `index` *(integer, required)* — 0-based index of the entry.

**Example.** Get value at index 2.
...
**Example.** Check if source IP is private.
...
**Example.** Check if source IP matches subnet.
...
**Example.** Extract parameter value.
...
Returns 1 if the parameter exists (with or without value), else 0.
...
Performs substitution using a POSIX regular expression.
...
**Example.** Replace 'a' with 'A' globally.
...
**Example.** Evaluate variables inside a string.
...
**Example.** Pad string with zeros to the left.
...
**Example.** Pad string with zeros to the right.
...
- `offset` *(integer, optional)* — Offset to begin search. Default is 0.
...
- `offset` *(integer, optional)* — Offset to start search before.
- `substring` *(string, required)* — String to search for.
...
- `separator` *(char, required)* — Character used to identify fields.

**Example.** Select field at index 1 using comma separator.
...
- `length` *(integer, required)* — Size of the substring. 0 or greater than string length returns substring to end.
- `offset` *(integer, required)* — Starting index. Negative values count from the end.

**Example.** Get substring starting at index 1 to the end.
...
**Example.** Expand string with spaces.
...
- `index` *(integer, optional)* — Line number of the type. Default is 0.
- `line_type` *(string, required)* — Type of the line (e.g., 'a', 'm', 'v').

**Example.** Get the second 'a' line.
...
- `index_or_type` *(string, required)* — Index of the stream or media type (e.g., 'audio').

**Example.** Get the audio stream.
...
- `index_or_type` *(string, required)* — Index of the stream or media type to delete.

**Example.** Delete video stream from SDP.
...
Returns the value of URI parameter with name.
...
Returns the value of Via header parameter with name.
