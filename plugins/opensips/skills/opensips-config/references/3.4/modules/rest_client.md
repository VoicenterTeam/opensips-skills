...
<!-- generated-from: data/3.4/modules/rest_client.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 rest_client module. Read this file when configuring or debugging the rest_client module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `txt` *(string, required)* — The contents of txt should adhere to the specification for HTTP headers (ex. Field: Value)
...
**Example.** Example of initializing a TLS client domain.
...
Perform a blocking HTTP POST on the given url. Note that the send_body parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.
...
- `retcode_pv` *(var, optional)* — Output variable which will retain the status code of the HTTP response. A 0 status code value means no HTTP reply arrived at all.
- `send_body` *(string, required)* — The request body.
...
Perform a blocking HTTP PUT on the given url. Similar to rest_post(), the send_body_pv parameter can also accept a format-string but it cannot be larger than 1024 bytes. For larger messages, you must build them in a pseudo-variable and pass it to the function.
