...
<!-- generated-from: data/3.4/modules/nat_traversal.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 nat_traversal module. Read this file when configuring or debugging the nat_traversal module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The name of the sharing tag (as defined per clusterer modules) to control which node is responsible for perform pinging of the contacts. If defined, only the node with active status of this tag will perform the pinging. The cluster_id must be defined for this option to work. This is an optional parameter. If not set, all the nodes in the cluster will individually do the pinging.
...
Returns a boolean value (0 or 1) indicating if dialog tracking will be enabled by the nat_traversal module. The nat_traversal module will always track the dialog (by calling create_dialog internally) unless told otherwise.
...
By setting this pv to 0 the nat_traversal module will not attempt to create the dialog.
...
Sets the time interval (in seconds) required to send a keepalive message to all the endpoints that need being kept alive.
...
Specifies what SIP method to use to send keepalive messages.
...
Specifies extra headers that should be added to the keepalive messages that are sent by the proxy.
...
Specifies a filename where information about the NAT endpoints and the conditions for which they are being kept alive is saved when OpenSIPS exits.
...
Sets the ID of the cluster the module is part of.
...
Sets the name of the sharing tag to control which node is responsible for perform pinging of the contacts.
...
Checks if the client is behind NAT based on the specified test flags.
...
Replaces the IP and port in the Contact header with the IP and port the SIP message was received from.
...
Triggers keepalive functionality for the source address of the request.
...
Demonstrates how to restore the sending socket when relaying messages to a given user agent in multi-proxy environments.
...
Demonstrates using the $source_uri pseudo-variable to set the received AVP for the registrar module.
...
Demonstrates using $source_uri in multi-proxy environments to indicate the NAT endpoint URI to the next proxy.
