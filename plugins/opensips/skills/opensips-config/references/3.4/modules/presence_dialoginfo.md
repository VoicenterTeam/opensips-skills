# presence_dialoginfo Module Reference
<!-- generated-from: data/3.4/modules/presence_dialoginfo.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 presence_dialoginfo module. Read this file when configuring or debugging the presence_dialoginfo module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
### `force_single_dialog` (integer)

By default the module aggregates all available dialog info into a single dialog-info document containing multiple "dialog" elements. If the phone does not support this, you can activate this parameter. If this parameter is set, only the dialog element with the currently most interesting dialog state will be put into the dialog-info document. Thus, the dialog-info element will contain only a single "dialog" element. The algorithm chooses the state based onf the following order of priority (least important first): terminated, trying, proceeding, confirmed, early.

*Default value is 0.*

**Notes:** I consider the "early" state more intersting than confirmed as often you might want to pickup a call if the originall callee is already busy in a call.
...
Set the force_single_dialog parameter to 1 to ensure only a single dialog element is included in the dialog-info document.
