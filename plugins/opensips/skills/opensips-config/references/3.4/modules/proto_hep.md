...
<!-- generated-from: data/3.4/modules/proto_hep.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 proto_hep module. Read this file when configuring or debugging the proto_hep module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `hep_id` *(string, required)* — the name of the hep_id defined in modparam section, specifying where to do the tracing.
...
Demonstrates setting the `hep_id` parameter with various destination, transport, and version configurations.
...
Demonstrates setting the `homer5_on` parameter.
...
Demonstrates setting the `homer5_delim` parameter.
...
Demonstrates setting the `hep_port` parameter.
...
Demonstrates setting the `hep_send_timeout` parameter.
...
Demonstrates setting the `hep_max_msg_chunks` parameter.
...
Demonstrates setting the `hep_async` parameter.
...
Demonstrates setting the `hep_async_max_postponed_chunks` parameter.
...
Demonstrates setting the `hep_capture_id` parameter.
...
Demonstrates setting the `hep_async_local_connect_timeout` parameter.
...
Demonstrates setting the `hep_async_local_write_timeout` parameter.
...
Demonstrates usage of the `correlate` function.
