...
<!-- generated-from: data/3.4/modules/carrierroute.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 carrierroute module. Read this file when configuring or debugging the carrierroute module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `next domain stored in AVP` — Otherwise
...
- `rewritten host stored in AVP` — Otherwise
...
This command activates the specified host, i.e. it sets its status to 1. It is only usable in file mode. Following options are possible: -d - the domain containing the host, -p - the prefix containing the host, -h - the host to be activated. Use the "null" prefix to specify an empty prefix.

**Parameters:**

- `options` *(string, required)* — A quoted string containing options: -d (domain containing the host), -p (prefix containing the host), -h (host to be activated). Use the "null" prefix to specify an empty prefix.
...
This command adds a route rule, it is only usable in file mode. Following options are possible: -d - the domain containing the host, -p - the prefix containing the host, -h - the host to be added, -w - the weight of the rule, -P - an optional rewrite prefix, -S - an optional rewrite suffix, -i - an optional hash index, -s - an optional strip value. Use the "null" prefix to specify an empty prefix.

**Parameters:**

- `options` *(string, required)* — A quoted string containing options: -d (domain containing the host), -p (prefix containing the host), -h (host to be added), -w (weight of the rule), -P (optional rewrite prefix), -S (optional rewrite suffix), -i (optional hash index), -s (optional strip value). Use the "null" prefix to specify an empty prefix.
...
- `options` *(string, required)* — A quoted string containing options: -d (domain containing the host), -p (prefix containing the host), -h (host to be deactivated), -t (new host used as backup). Use the "null" prefix to specify an empty prefix.
...
This command delete the specified hosts or rules, i.e. remove them from the route tree. It is only usable in file mode. Following options are possible: -d - the domain containing the host, -p - the prefix containing the host, -h - the host to be added, -w - the weight of the rule, -P - an optional rewrite prefix, -S - an optional rewrite suffix, -i - an optional hash index, -s - an optional strip value. Use the "null" prefix to specify an empty prefix.
...
This command reloads the routing data from the data source. Important: When new domains have been added, a restart of the server must be done, because the mapping of the ids used in the config script cannot be updated at runtime at the moment. So a reload could result in a wrong routing behaviour, because the ids used in the script could differ from the one used internally from the server. Modifying of already existing domains is no problem.

**Returns:** null

### `cr_replace_host`

This command can replace the rewrite_host of a route rule, it is only usable in file mode. Following options are possible: -d - the domain containing the host, -p - the prefix containing the host, -h - the host to be replaced, -t - the new host. Use the "null" prefix to specify an empty prefix.

**Parameters:**

- `options` *(string, required)* — A quoted string containing options: -d (domain containing the host), -p (prefix containing the host), -h (host to be replaced), -t (new host). Use the "null" prefix to specify an empty prefix.
