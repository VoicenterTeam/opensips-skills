...
<!-- generated-from: data/3.4/modules/event_routing.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 event_routing module. Read this file when configuring or debugging the event_routing module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The scenario is Alice calling to bob, Bob does not pickup and Charlie is performing call pickup (to get the call from Alice). We use `notify_on_event` to link the two calls: the one from Alice to Bob and the one from Charlie to call pickup service.
