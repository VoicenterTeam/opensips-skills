...
<!-- generated-from: data/3.4/modules/event_stream.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 event_stream module. Read this file when configuring or debugging the event_stream module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
This module provides a TCP transport layer implementation for the Event Interface. The module can either send a JSON-RPC notification or a standard request and wait for the response (when used in reliable_mode).
...
*   _'tcp:'_ - specifies the transport protocol used by the Event Interface to send the command. the _tcp_ token indicates that the subscriber's events should be notified using the _event_stream_ module.
...
**Example.** Set the `event_param` parameter.
...
**Example.** Set the `reliable_mode` parameter.
