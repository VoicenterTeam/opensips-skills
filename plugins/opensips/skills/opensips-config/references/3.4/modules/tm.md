# tm Module Reference
<!-- generated-from: data/3.4/modules/tm.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 tm module. Read this file when configuring or debugging the tm module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `clusterer` — if the anycast scenario is enabled (optional)
...
**Example.** Set the `cluster_auto_cancel` parameter.
...
**Example.** Set the `cluster_param` parameter.
...
Timeout which is triggered if no final reply for an INVITE arrives after a provisional message was received (in seconds). This timeout starts counting down once the first provisional response is received. Thus, fast failover (no 100 trying from gateway) can be achieved by setting _[fr_timeout](#param_fr_timeout "1.3.1.fr_timeout (integer)")_ to low values. See example below.
...
- `reason_hdr` *(string, required)* — A fully formated Reason header with name, body and CRLF.

**Related:**

- `t_relay`

**Example.** Enforce a custom Reason header in a CANCEL request..
...
- `sip_hdrs` *(string, required)* — A set of headers to be attached to the transaction.

**Example.** Attach a custom X-origin header to the transaction..
...
**Example.** Cancels the current branch if a 183 reply is received.
...
**Example.** Checks if the reply code is 487 or 408.
...
- `true` — if the current request is associated to a transaction
- `false` — if the request is not a retransmission (for non-CANCEL/non-ACK)
- `-2` — for end-to-end ACKs (to 2xx replies from a different transaction)
...
**Example.** Example 1.27. t_check_trans usage.
...
**Usable from:** timer route, event route, notification route
...
**Example.** Injects branches from an event notification..
...
- `reply` *(string, required)* — Specifies which replies to check (all or last)
  - `all`
  - `last`
...
- `false` — If replies were received

**Usable from:** REQUEST_ROUTE, BRANCH_ROUTE, FAILURE_ROUTE, ONREPLY_ROUTE

**Example.** Checks if all replies were locally generated.
...
**Example.** Sends a new MESSAGE request.
...
**Example.** Example 1.26. t_newtran usage.
...
**Example.** Sets branch route block 1 to filter or modify branches..
...
**Example.** Sets failure route block 1 and relays the request..
...
**Example.** Sets reply routes globally and per branch..
...
- `flags` *(string, optional)* — Comma separated list of string flags for controlling the internal behaviour.
  - `no-auto-477`
  - `no-dns-failover`
  - `pass-reason-hdr`
  - `allow-no-cancel`
- `outbound_proxy` *(string, optional)* — String representing an outbound proxy (a fixed destination) where the message should be sent. The destination is specified as “[proto:]host[:port]”. If a destination URI “$du” for this message was set before the function is called then this value will be used as the destination instead of the function parameter.
...
**Example.** Example 1.23. t_relay usage.
...
- `flags` *(string, optional)* — A set of flags for controlling the internal behaviour. Only no-dns-failover is applicable here.
  - `no-dns-failover`
  - `0x04`
- `uri` *(string, required)* — The destination specified by a SIP URI.
...
**Example.** Replicate requests to various destinations with different transport and flags..
...
**Example.** Example 1.24. t_reply usage.
...
- `callid` *(string, optional)* — Call-ID used to identify the transaction. If omitted, obtained from current message.
- `code` *(int, required)* — Reply code number.
- `cseq` *(int, optional)* — CSeq number used to identify the transaction. If omitted, obtained from current message.
- `reason_phrase` *(string, required)* — Reason string.
...
**Example.** Example 1.25. t_reply_with_body usage.
...
**Example.** Prepares the transaction to wait for dynamically injected branches..
...
**Example.** Stop waiting for new branches in a transaction..
...
- `false` — If the transaction was not cancelled

**Usable from:** ONREPLY_ROUTE, FAILURE_ROUTE

**Example.** Logs if the transaction was cancelled.
...
- `fifo` *(string, required)* — Path to the FIFO file.
- `info` *(string, required)* — Information to be written, controlled via the tw_append parameter.

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE

**Example.** Write request information to a FIFO file using a predefined append format..
...
- `info` *(string, required)* — Information string to be written (controlled via tw_append parameter).
...
- **Scope:** transaction, onreply_route
### `$T_fr_timeout`
