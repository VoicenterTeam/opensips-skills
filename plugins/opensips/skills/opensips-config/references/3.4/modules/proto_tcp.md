...
<!-- generated-from: data/3.4/modules/proto_tcp.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 proto_tcp module. Read this file when configuring or debugging the proto_tcp module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The default port to be used for all TCP related operation.
...
The maximum number of chunks that a SIP message is expected to arrive via TCP.
...
If the TCP connect and write operations should be done in an asynchronous mode (non-blocking connect and write).
...
If _tcp_async_ is enabled, this specifies the maximum number of SIP messages that can be stashed for later/async writing.
...
If _tcp_async_ is enabled, this specifies the number of milliseconds that a connect will be tried in blocking mode (optimization).
...
If _tcp_async_ is enabled, this specifies the number of milliseconds that a write op will be tried in blocking mode (optimization).
...
This controls whether tracing for tcp is on or not.
