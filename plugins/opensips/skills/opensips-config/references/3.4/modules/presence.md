...
<!-- generated-from: data/3.4/modules/presence.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 presence module. Read this file when configuring or debugging the presence module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `signaling` — The following modules must be loaded before this module

### External Libraries

- `libxml-dev` — External Libraries or Applications
...
**Notes:** This parameter has meaning only in clustering mode.
...
Comma Separated Value (CSV) list with the events to considered by the federated cluster - only presentities advertising one of these events will be broadcasted via the cluster. For more on presence clustering see the [Section 1.2, “Presence clustering”](#presence_clustering "1.2. Presence clustering") chapter.
...
**Notes:** Default value is enabled.
...
Exposes in the script, by rasing an _E_PRESENCE_EXPOSED_ event, all the presentities of a specific event that match a specified filter.
