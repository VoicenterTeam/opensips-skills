...
<!-- generated-from: data/3.4/modules/presence_xcapdiff.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 presence_xcapdiff module. Read this file when configuring or debugging the presence_xcapdiff module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
The presence_xcapdiff module adds support for the "xcap-diff" event to presence and pua. The module registers the event but doesn't do any event-specific processing. The event is triggered when some modification of a document happens, allowing the XCAP server to publish the xcap-event.

**Subscribe via:** Registering the event with presence allows clients to subscribe to the event.
