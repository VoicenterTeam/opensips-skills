...
<!-- generated-from: data/3.4/modules/rabbitmq_consumer.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 rabbitmq_consumer module. Read this file when configuring or debugging the rabbitmq_consumer module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `librabbitmq-dev` — Required for module operation. Not compatible with versions 0.4 or below.
...
*   _tls_domain_ - indicates which TLS domain (as defined using the _tls_mgm_ module) to use for this connection. This must be an _amqps_ URI and the [use_tls](#param_use_tls "1.3.4.use_tls (integer)") module parameter must be enabled.
...
Setting this parameter will allow you to use TLS for broker connections. In order to enable TLS for a specific connection, you can use the "tls_domain=_dom_name_" parameter in the configuration specified through the [connection_id](#param_connection_id "1.3.1.connection_id (string)") module parameter.
...
Demonstrates setting the `connection_id` parameter for various scenarios including basic connection, message acknowledgment, and TLS.
...
Demonstrates setting the `use_tls` parameter along with necessary TLS management module parameters.
