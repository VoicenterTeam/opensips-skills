...
<!-- generated-from: data/3.4/modules/rate_cacher.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 rate_cacher module. Read this file when configuring or debugging the rate_cacher module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `client_id` *(string/integer, required)* — The client_id originating this call
- `desired_margin` *(string/integer, required)* — The minimum Integer margin that the script writer wants to achieve, based on the Client sell and Vendor buy prices. The formula used is : vendor_margin=(client_price - results[i])*100/client_price) . If the vendor_margin is higher than the desired_margin, then the Vendor is ok to use. The desired margin can be positive ( call will be profitable ) or negative ( the call will cause a loss ).
- `dialled_no` *(string/integer, required)* — The DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `is_wholesale` *(string/integer, required)* — Contains either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning )
  - `1`
  - `0`
- `out_vendor_csv` *(string/integer, required)* — An output parameter, and the pvar will get populated with the CSV list of Vendors that meet the desired margin condition
- `vendors_csv` *(string/integer, required)* — A list of Vendors that need to be filtered based on the desired margin ( keep just those that match your desired percentage margin for this call )
...
- `client_id` *(string/integer, required)* — The client_id originating this call
- `destination_pvar` *(string/integer, required)* — Will contain the matched destination from the client's ratesheet
- `dialled_no` *(string/integer, required)* — The DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string/integer, required)* — Will contain the matched increment from the client's ratesheet
- `is_wholesale` *(string/integer, required)* — Contains either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning )
  - `1`
  - `0`
- `minimum_pvar` *(string/integer, required)* — Will contain the matched minimum from the client's ratesheet
- `prefix_pvar` *(string/integer, required)* — Will contain the matched prefix from the client's ratesheet
- `price_pvar` *(string/integer, required)* — Will contain the matched price from the client's ratesheet
...
- `destination_pvar` *(string/integer, required)* — Will contain the matched destination from the vendor's ratesheet
- `dialled_no` *(string/integer, required)* — The DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string/integer, required)* — Will contain the matched increment from the vendor's ratesheet
- `minimum_pvar` *(string/integer, required)* — Will contain the matched minimum from the vendor's ratesheet
- `prefix_pvar` *(string/integer, required)* — Will contain the matched prefix from the vendor's ratesheet
- `price_pvar` *(string/integer, required)* — Will contain the matched price from the vendor's ratesheet
- `vendor_id` *(string/integer, required)* — The vendor_id
...
**Returns:** Ratesheet information including prefix, destination, price, minimum, increment, and currency. (structured response — see schema)
