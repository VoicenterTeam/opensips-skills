...
<!-- generated-from: data/3.4/modules/dispatcher.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 dispatcher module. Read this file when configuring or debugging the dispatcher module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Starting with version 2.1, the dispatcher module keeps its destination sets into different partitions. Each partition is described by its own "db_url", "table_name", "dst_avp", "grp_avp", "cnt_avp", "sock_avp", "attr_avp" and "blacklists" set of attributes. Setting any of these module parameters will only alter the "default" partition's properties.
...
**Example.** Set the `algo_route` parameter.
...
**Example.** Set the `attrs_avp` parameter.
...
Defines a blacklist based on a dispatching setid from the 'default' partition. This list will contain the IPs (no port, all protocols) of the destinations matching the given setid. Use the 'partition' parameter if you want to define blacklists based on other partitions' sets. Multiple instances of this param are allowed.
...
**Notes:** This method is only available, if compiled with the probing of failed gateways enabled.

**Example.** sip:proxy@sip.somehost.com.
...
**Notes:** This parameter is only used, when the TM-Module is loaded.
...
**Notes:** The accepted values are any positive integer values, including the “0” value.
...
The dynamic weights are recalculated every _event_heartbeat_interval_ seconds (see the "freeswitch" OpenSIPS module for more details regarding this setting), as the stats from FreeSWITCH are expected to arrive. The update formula is shown below (FreeSWITCH stats are highlighted in bold):
...
**Example.** Set the `hash_pvar` parameter.
...
Define a new partition (data source) with the following properties: "db_url", "table_name", "dst_avp", "grp_avp", "cnt_avp", "sock_avp", "attrs_avp", "script_attrs", "ds_define_blacklist". All these properties are optional, having appropriate default values.

The syntax is: "partition_name: param1 = value1; param2 = value2". Each value format is the same as the one used to define a specific parameter using modparam.
...
**Example.** Set the `probe_mode_col` parameter.
...
**Example.** Set the `script_attrs_avp` parameter.
...
**Example.** Set the `socket_col` parameter.
...
**Example.** Set the `weight_col` parameter.
...
- `set` *(integer, required)* — a set of dispatching destinations
- `state_filter` *(string, required)* — which destinations should be counted. Either active ("a", "A" or "1"), inactive ("i", "I" or "0"), probing ("p", "P" or "2") destinations or different combinations between these flags, such as "pI", "1i", "ipA"...

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, LOCAL_ROUTE, TIMER_ROUTE, EVENT_ROUTE

**Example.** ds_count usage examples.
...
- `out_attrs` *(pvar, required)* — name of a variable where we will store the script attrs.
- `partition` *(string, optional)* — name of a DB partition. If omitted, the "default" partition will be used.
- `set` *(integer, required)* — Setid for which we are pushing script attributes
- `uri` *(string, required)* — URI address for which we are getting script attributes

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, LOCAL_ROUTE, TIMER_ROUTE, EVENT_ROUTE

**Example.** ds_get_script_attrs usage example (Note: source text repeats ds_push_script_attrs example here).
...
- `active_only` *(integer, optional)* — specify a non-zero value in order to only search through the active destinations (ignore the ones in probing and inactive states)
...
- `port` *(integer, required)* — a port to test against the dispatcher list. Use a 0 value in order to match any port
- `set` *(integer, optional)* — a dispatcher set identifier to test against. If missing, all sets will be checked. The -1 set is a special value, acting as a "check all sets" wildcard.

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, ONREPLY_ROUTE

**Example.** ds_is_in_list usage examples.
...
Mark the last used address from partition's destination set as inactive ("i"/"I"/"0"), active ("a"/"A"/"1") or probing ("p"/"P"/"2"). When an address is marked as inactive or probing, it will be ignored by ds_select_dst() and ds_select_domain().
...
Takes the next destination address from the AVPs with id partition.'dst_avp_id' and sets the domain part of the request uri. This function is using the flags set in ds_select_dst or ds_select_domain.
...
Takes the next destination address from the AVPs with id partition.'dst_avp_id' and sets the dst_uri (outbound proxy address). This function is using the flags set in ds_select_dst or ds_select_domain.
...
- `script_attr` *(string or pvar, required)* — The new script attributes
- `set` *(integer, required)* — Setid for which we are pushing script attributes

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, LOCAL_ROUTE, TIMER_ROUTE, EVENT_ROUTE

**Example.** ds_push_script_attrs usage example.
...
The method selects a destination from addresses set and rewrites the hostname and port parts of the Request-URI ($ru). Its parameters have same meaning as in ds_select_dst().

**Parameters:**

- `alg` *(integer, required)* — the algorithm used to select the destination address
- `flags` *(string, optional)* — a string of flag-settings which tweak the function's behavior
- `max_res` *(integer, optional)* — signifies that only a maximum number of destinations shall be included in the specified failover AVP
- `partition` *(string, optional)* — name of a DB partition
- `set` *(integer, required)* — a set identifier from which to select destinations
...
The method selects a destination from the given set of addresses. It will overwrite the destination URI ($du) of a SIP request.
...
- `false` — if no active destinations are found
...
**Example.** ds_select_dst usage examples with various parameters and variables.
...
The default DB connection of the module, overriding the global 'db_default_url' setting.
...
The name of the avp to contain the attributes string of the current destination.
...
Name of the avp to contain the script attributes string of the current destination.
...
The name of the PV where to store the set ID (group ID) when calling ds_is_in_list() without group parameter.
...
This parameter allows you to enforce a specific Max-Forward value for the SIP pinging requests generated by the Dispatcher modules.
...
A socket description [proto:]host[:port] of the local socket to be used for sending the probing messages from.
...
Defines a list of one or more setids that limits which destinations are probed if probing is active.
...
Defines a blacklist based on a dispatching setid from the 'default' partition.
...
This parameter must contain a list of SIP reply codes separated by comma.
...
This is mainly for internal usage and represents the name of the avp which will hold the list with addresses.
...
This is mainly for internal usage and represents the name of the avp storing the group id of the destination set.
...
This parameter is used by the PVAR(9) algorithm to specify the pseudovariable pattern used to detect the load of each destination.
...
The ID of the cluster the module is part of.
...
The name of the sharing tag (as defined per clusterer modules) to control which node is responsible for perform the self-triggered actions in the module.
...
This paramter controls how the probing/pinging should be done when using the clustering support.
...
The default name of the table from which to load dispatcher destinations.
...
If enabled, FreeSWITCH destinations may have dynamic dispatching weights, refreshed at runtime.
...
The method selects a destination from the given set of addresses.
...
Returns the number of active, inactive or probing destinations in a partition's set.
