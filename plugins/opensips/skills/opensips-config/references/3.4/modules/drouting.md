...
<!-- generated-from: data/3.4/modules/drouting.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 drouting module. Read this file when configuring or debugging the drouting module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Notes:** The supported probing modes are:

*   **"all"** - all the nodes in the cluster will independetly ping all the defined gateways, an "all" pings "all" mode.
    
*   **"by-shtag"** - all the gateways are pinged by only one node in the cluster, the node having the [cluster_sharing_tag](#param_cluster_sharing_tag "1.3.25.cluster_sharing_tag (string)") active. By activating the sharing tag on a different node, the pinging duty will be transfered to another node in the cluster.
...
- `true` — only if a new RURI was set.
...
**Example.** Verify status change when use_partitions is 0.
...
**Example.** Get status of carrier 'CR1' in 'my_partition' when use_partitions is 1.
...
**Example.** Set status of carrier 'CR1' in 'partition_1' to enabled when use_partitions is 1.

```opensips-cli
$ opensips-cli -x mi dr_carrier_status partition_name=partition_1 carrier_id=CR1 status=1
```

**Example.** Verify status change when use_partitions is 1.
...
**Example.** Get status of gateway with ID 2 when use_partitions is 0.
...
**Example.** Set status of gateway with ID 2 to disabled when use_partitions is 0.
...
**Example.** Check status of gateway with ID 3 when use_partitions is 0.
...
**Example.** Test routing for number '012340987' in group '3' of partition 'part1'.
...
Command to reload routing rules from database. If use_partition is set to 0, all routing rules will be reloaded. If use_partition is set to 1, the partition_name parameter may be used to reload a specific partition.
...
**Example.** Reloads the partition named 'part_1'.
