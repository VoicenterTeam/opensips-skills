...
<!-- generated-from: data/3.4/modules/load_balancer.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 load_balancer module. Read this file when configuring or debugging the load_balancer module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `grp` *(integer, required)* — group id for the destinations; if no knows, "-1" will mean all groups.
- `ip` *(string, required)* — IP to identify the destination the call has to be counted for.
- `port` *(integer, required)* — PORT to identify the destination the call has to be counted for.
- `resources` *(string, required)* — a semi-colon separated list of resources required by the current call.
- `undo` *(integer, optional)* — if set to a non zero value, it will force the function to un-count - actually it will undo the counting of this call as load in the current LB session; this might be needed if we count call for particular resources and then need to un-count it.
...
- `active` *(integer, optional)* — if "1", the search will be performed only over "active" (not disabled) destinations. If missing, the search will consider any kind of destinations.
- `attrs` *(var, optional)* — a writable variable to be populated with the attributes of the identified destination.
- `group` *(integer, optional)* — in what LB group the destination should be looked for; If not specified, the search will be in all groups.
- `ip` *(string, required)* — IP to be checked
- `port` *(integer, required)* — PORT to be checked. A value 0 means "any" - will match any port.
...
Function to be used to pull the next available (and less loaded) destination. You need to have an ongoing LB session (started with lb_start()). This function is mainly used for implementing failover for the LB destinations.
...
- `1` — if a new destination URI is set, pointing to the selected destination. NOTE that the RURI will not be changed by this function.
- `-1` — generic internal error (memory allocation, parsing)
- `-2` — no capacity available (detinations are up and available, but they do not have any availabe channels)
- `-3` — no more destinations available (the requested resources did not match any active destination)
...
- `flags` *(string, optional)* — various flags to controll the LB algorithm ( or computing the available load on the system)
  ...
  - `r`
  - `s`
- `grp` *(integer, required)* — group id for the destinations; the destination may be grouped in several groups you can you for differnet scenarios.
- `resources` *(string, required)* — a semi-colon separated list of resources required by the current call.
...
- `-1` — generic internal error (memory allocation, parsing)
- `-2` — no capacity available (detinations are up and available, but they do not have any availabe channels)
- `-3` — no destinations available (the requested resources did not match any active destination)
- `-4` — bad resources (requested resources do not exist)
...
- `attrs` *(var, optional)* — a writable variable to be populated with the attributes of the selected destination.
- `flags` *(string, optional)* — various flags to controll the LB algorithm
- `grp` *(integer, required)* — group id for the destinations
- `resources` *(string, required)* — a semi-colon separated list of resources required by the current call.
...
- `destination_id` *(integer, required)* — the ID (as per DB) of the destination.
...
...
```
### Set `initial_freeswitch_load` parameter
