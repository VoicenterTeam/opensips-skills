...
<!-- generated-from: data/3.4/modules/db_virtual.json
     generator-version: 0.1.0
     opensips-version: 3.4
     doc-type: module -->

Reference for the OpenSIPs 3.4 db_virtual module. Read this file when configuring or debugging the db_virtual module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `db_url_index` *(int, required)* — 
- `ignore_retries` *(boolean, optional)* — 
- `may_use_db_flag` *(boolean, required)* — 
- `set_index` *(int, required)* — 

**Example.** db_set 3 2 0 1 means:
* 3 - the fourth set (must exist)
* 2 - the third URL in the fourth set(must exist)
* 0 - processes are not allowed to use that URL
* 1 - reset and suppress db_max_consec_trys
