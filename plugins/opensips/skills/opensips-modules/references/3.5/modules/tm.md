# tm Module Reference
<!-- generated-from: data/3.5/modules/tm.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 tm module. Read this file when configuring or debugging the tm module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Example.** Set the `disable_6xx_block` parameter.
...
**Example.** Set the `onreply_avp_mode` parameter.
...
**Example.** Set the `pass_provisional_replies` parameter.
...
**Example.** Set the `syn_branch` parameter.
...
*   _tw_append = append_name':' element (';'element)*_
...
**Example.** Set the `tw_append` parameter.
...
**Example.** Replicates ACK or CANCEL if no local transaction is found..
...
- `re` *(string, required)* — regular expression to match the reply code
...
**Example.** Flushes flags to the transaction..
...
- `flags` *(string, optional)* — some additional flags related to the injection process.
  ...
  - `c`
  - `l`
- `source` *(string, required)* — where to take the description for the new branches to be injected. It can be event or msg.
...
- `code` *(integer, required)* — Reply code number.
...
- `callid` *(string, optional)* — The Call-ID used to identify the transaction. If omitted, it is obtained from the current message.
- `code` *(int, required)* — Reply code number
- `cseq` *(int, optional)* — The CSeq used to identify the transaction. If omitted, it is obtained from the current message.
- `reason_phrase` *(string, required)* — Reason string
...
**Example.** Handling a parked INVITE and replying by Call-ID for CANCEL and BYE..
...
- `info` *(string, required)* — Which information should be written may be control via the “tw_append” parameter.
...
- `info` *(string, required)* — Information identifier
...
**Example.** Writes transaction info to a UNIX socket..
