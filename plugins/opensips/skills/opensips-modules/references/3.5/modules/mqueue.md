...
<!-- generated-from: data/3.5/modules/mqueue.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 mqueue module. Read this file when configuring or debugging the mqueue module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Demonstrates setting the `db_url` parameter and defining a database table schema.
...
Demonstrates defining memory queues with various attributes such as size and addmode.
...
Demonstrates adding a new item to a queue.
...
Demonstrates fetching items from a queue and logging them.
...
Demonstrates freeing the item fetched in pseudo-variables.
...
Demonstrates retrieving and logging the current number of elements in a queue.
...
Demonstrates getting the size of a memory queue via the Management Interface.
...
Demonstrates fetching a key-value pair from a memory queue via the Management Interface.
...
Demonstrates getting the size for all memory queues via the Management Interface.
