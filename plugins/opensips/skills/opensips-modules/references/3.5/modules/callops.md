...
<!-- generated-from: data/3.5/modules/callops.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 callops module. Read this file when configuring or debugging the callops module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Demonstrates how to drop automatically handled NOTIFY refer events to prevent them from reaching the end-point.
...
Demonstrates setting the `mode` parameter to manual.
...
Demonstrates setting the `match_param` parameter.
...
Demonstrates using the `call_blind_replace()` function to match an existing leg when manual mode is used.
...
Demonstrates using the `call_transfer_notify()` function to handle NOTIFY refer requests.
...
Demonstrates using the `call_transfer()` function to perform a blind transfer of the caller to a new destination.
...
Demonstrates using the `call_transfer()` function to perform an attended transfer of the caller to the callee of a different call.
