...
<!-- generated-from: data/3.5/modules/media_exchange.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 media_exchange module. Read this file when configuring or debugging the media_exchange module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `body` *(string, required)* — The body has to be specified in the mandatory body parameter
- `callid` *(string, required)* — the callid of the dialog that will have it's leg mixed with the new call to the Media Server
- `headers` *(string, optional)* — extra headers to add to the outgoing request
- `leg` *(string, required)* — indicates the participant that will have its media pined into the new call. Possible values are “caller” and “callee”.
- `nohold` *(integer, optional)* — if set to a non-zero value, the module avoids putting the other participant on hold when the media exchanging starts
