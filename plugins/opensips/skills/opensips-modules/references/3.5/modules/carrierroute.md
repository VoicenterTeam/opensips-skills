...
<!-- generated-from: data/3.5/modules/carrierroute.json
     generator-version: 0.1.0
     opensips-version: 3.5
     doc-type: module -->

Reference for the OpenSIPs 3.5 carrierroute module. Read this file when configuring or debugging the carrierroute module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `tm module` — when you want to use the $T_reply_code pseudo-variable in the “cr_next_domain” function.
...
**Example.** Set the `carrier_column` parameter.
...
**Example.** Set the `carrier_id_col` parameter.
...
**Example.** Set the `carrier_name_col` parameter.
...
**Example.** Set the `id_column` parameter.
...
**Example.** Set the `scan_prefix_column` parameter.
...
**Example.** Set the `subscriber_domain_col` parameter.
...
**Example.** Set the `subscriber_table` parameter.
...
**Example.** Set the `subscriber_user_col` parameter.
...
- `-i` *(string, optional)* — an optional hash index
- `-p` *(string, required)* — the prefix containing the host
- `-P` *(string, optional)* — an optional rewrite prefix
- `-s` *(string, optional)* — an optional strip value
- `-S` *(string, optional)* — an optional rewrite suffix
- `-w` *(string, required)* — the weight of the rule
