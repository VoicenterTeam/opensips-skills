...
<!-- generated-from: data/3.6/modules/call_center.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 call_center module. Read this file when configuring or debugging the call_center module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Notes:** If not explicitly set, the global OpenSIPS DB URL will be used.
...
**Notes:** NOTE that you can change the value of the current scenario by writing into it, but the change will not be reflected in a different scenario.
...
**Example.** Set the `cc_flows_table` parameter.
...
**Example.** Set the `cca_wrapupend_column` parameter.
...
**Example.** Set the `cca_wrapuptime_column` parameter.
...
**Example.** Set the `ccf_flowid_column` parameter.
...
**Example.** Set the `ccf_priority_column` parameter.
...
- `flow_id` *(integer)* — the flow ID that delivered the call for this agent; published only if the state is "incall"
...
This example shows how to set a custom context parameter name and how to retrieve the passed parameter value in a subsequent B2B scenario.
...
This function must be used only for initial INVITE requests in REQUEST_ROUTE.
### Example 1.34. `cc_agent_login` usage

Using cc_agent_login in REQUEST_ROUTE to log off the agent named 'agentX'.
...
Demonstrating how to retrieve RTP statistics using $rtpquery and logging the totals.
...
This example is listed under Pseudo-Variables but shows usage of $rtpquery which is related to statistics/data retrieval.
