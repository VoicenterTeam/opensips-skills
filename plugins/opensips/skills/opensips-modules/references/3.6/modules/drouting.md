...
<!-- generated-from: data/3.6/modules/drouting.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 drouting module. Read this file when configuring or debugging the drouting module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
*   The value stored in database has the basic format of: <timezone>|<dtstart>|<dtend>|<duration>|<freq>|<until>|<interval>|<byday>|<bymonthday>|<byyearday>|<byweekno>|<bymonth> , identical to the input of the [check_time_rec()](cfgutils#func_check_time_rec) function of the _cfgutils_ module, including the optional use of logical operators linking multiple such strings into a larger expression.
...
**Example.** Set the `drc_table` parameter.
...
**Example.** Set the `drd_table` parameter.
...
**Example.** Set the `drg_table` parameter.
...
**Example.** Set the `drr_table` parameter.
...
**Notes:** NOTE: If the restart persistent cache is not populated from a previous run, then the data will be loaded from database at startup!
...
- `carrier_attrs_pvar` *(var, optional)* — a writable variable which will be populated with the attributes of the matched carrier
- `flags` *(string, optional)* — a list of letter-like flags for controlling the routing behavior. F: Enable rule fallback; L: Do strict length matching; C: Only check if the dialed number matches any routing rule
  - `F`
  - `L`
  - `C`
...
- `gw_whitelist` *(string, optional)* — a comma separated white list of gateways. This will force routing over, at most, this list of carriers or gateways
- `partition` *(string, optional)* — the name of the DR partition to be used. Mandatory if use_partitions is 1. Wildcard '*' can be used to force routing over all partitions
...
```opensips
do_routing(0, "F", , , $var(gw_attributes));
```

### `dr_disable([partition])`

Marks as disabled the last destination that was used for the current call. The disabling done via this function will prevent the destination to be used for usage from now on. The probing mechanism can re-enable this peer.

**Parameters:**

- `partition` *(string, optional)* — the name of the DR partition to be used. Mandatory if use_partitions is 1. Wildcard sign is accepted

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, ONREPLY_ROUTE, LOCAL_ROUTE

**Example.** Disable gateway on timeout or 5xx error.
...
- `flags` *(string, optional)* — letter like flags: 's' (Strip), 'p' (Prefix), 'i' (Gateway ID), 'n' (Ignore port), 'c' (Carrier ID)
  - `s`
  - `p`
  - `i`
  - `n`
...
- `type` *(int, optional)* — number for the GW/destination type to be checked; when omitting this parameter or specifying -1, matching will be done against all types
...
- `flags` *(string, optional)* — letter-like flags: L (Do strict length matching over the prefix)
  - `L`
...
- `partition` *(string, optional)* — the name of the DR partition to be used. Mandatory if use_partitions is 1
- `rule_attrs_pvar` *(var, optional)* — a writable variable which will be populated with the attributes of the matched dynamic routing rule

**Usable from:** REQUEST_ROUTE, FAILURE_ROUTE, BRANCH_ROUTE, ONREPLY_ROUTE, LOCAL_ROUTE

**Example.** Check if From username matches rules in group 1 of partition 'dids' using strict length matching.
...
- `carrier_attrs_pvar` *(var, optional)* — an output writable variable which will be populated with the attributes of the matched carrier
- `flags` *(string, optional)* — letter like flags: 's' (Strip), 'p' (Prefix), 'i' (Gateway ID), 'n' (Ignore port), 'r' (Check protocol), 'c' (Carrier ID)
  - `s`
  - `p`
  - `i`
  - `n`
...
**Example.** match the source IP (only) against all gateways.
...
- `partition` *(string, optional)* — the name of the DR partition to be used. Mandatory if use_partitions is 1. Wildcard sign is not accepted
...
**Example.** use_partitions is not set, routing to a list of gateways.
...
- `carrier_id` *(string, optional)* — The id of a carrier. If provided, the function will return/set the status of that carrier, otherwise it will list all carriers along with their statuses.
...
**Returns:** Returns the enabled status of the carrier (e.g., 'Enabled:: no', 'Enabled:: yes').
...
**Returns:** Returns the current probing status (e.g., 'Status:: 1').
...
- `gw_id` *(string, optional)* — The id of a gateway. If provided, the function will return/set the status of that gateway, otherwise it will list all gateways along with their statuses.
- `partition_name` *(string, optional)* — Required if use_partitions is set to 1.
- `status` *(integer, optional)* — The new status to be forced for a GW (0 - disable, 1 - enable). Only makes sense if gw_id is provided.

**Returns:** Returns the state or enabled status of the gateway (e.g., 'State:: Active', 'Enabled:: Disabled MI').
...
**Example.** Get status of gateway 3 when use_partitions is 0.
...
**Example.** Get status of partition 'partition8' when use_partitions is 1.
...
**Returns:** Returns the matched prefix along with the list of the gateways / carriers.
...
**Returns:** Not specified in documentation

**Example.** Reloads the partition 'part_1'.
...
**Returns:** Returns the date of the last reload (e.g., 'Date:: Tue Aug 12 12:26:00 2014').
...
Sets the database URL for the drouting module.
...
Sets the name of the db table storing gateway addresses.
...
Sets the name of the db table storing routing rules.
...
Sets the name of the db table storing groups.
...
Sets the name of the db table storing definitions of the carriers that will be used directly by the routing rules.
...
Sets the name of the avp for storing Request URIs to be later used (alternative destiantions for the current one).
...
Sets the name of the avp for storing the id of the current selected gateway/destination - once a new destination is selected (via the use_next_gw() function), the AVP will be updated with the ID of the new selected gateway/destination.
...
Sets the name of the avp for storing the PRI prefix of the current selected destination/gateway - once a new destination is selected (via the use_next_gw() function), the AVP will be updated with the PRI prefix of the new used destination.
...
Sets the name of the avp for storing the id of the current matched routing rule (see dr_rules table).
...
Sets the actual prefix that matched the routing rule (the part from RURI username that matched the routing rule).
...
Sets the name of the avp for storing sockets for alternative destinations defined by ruri_avp.
...
A socket description [proto:]host[:port] of the local socket (which is used by OpenSIPS for SIP traffic) to be used (if multiple) for sending the probing messages from. For probing gateway the highest priority has socket from gateway configuration in dr_gateways table. Then socket from global `probing_socket` parameter and the lowest priority is default behaviour with auto selected socket wich OpenSIPS listens on.
...
This paramter controls how the probing/pinging should be done when using the clustering support. It is about which node in the cluster pings which gateway/destination. The [cluster_id](#param_cluster_id "1.3.24.cluster_id (integer)") must be defined for this option to work.
...
Variable which will store the name of the name partition when _wildcard(*)_ operatior is used. _Use_partitions_ must be set in order to use this parameter. NOTE: The variable must be WRITABLE!
...
modparam("drouting", "rule_tables_query", "
	MY_RULES_QUERY:
		SELECT 'dr_rules_a' UNION SELECT 'dr_rules_b'")
```
### Example 1.39. Set the `generate_data_checksum` parameter
...
Demonstrates various ways to call the do_routing function, including parameter variations for use_partitions setting.
...
Demonstrates calling route_to_carrier with and without use_partitions enabled.
...
Demonstrates calling route_to_gw with and without use_partitions enabled, including fetching attributes.
...
Demonstrates calling use_next_gw with and without use_partitions enabled, including fetching attributes.
...
Demonstrates calling goes_to_gw with and without use_partitions enabled.
...
Demonstrates calling is_from_gw with and without use_partitions enabled.
...
Demonstrates calling dr_is_gw, checking against all gateways or specific partition.
...
Demonstrates calling dr_disable() without and with partition specified.
...
Demonstrates calling dr_match with different parameters.
...
Shows MI command usage for checking and setting gateway status without partitions.
...
Shows MI command usage for checking and setting gateway status with partitions.
...
Shows MI command usage for checking and setting carrier status without partitions.
...
Shows MI command usage for checking and setting carrier status with partitions.
...
Shows MI command usage for checking last reload time without partitions.
...
Shows MI command usage for checking last reload time with partitions.
...
Shows MI command usage for checking and setting probing status.
