# sst Module Reference
<!-- generated-from: data/3.6/modules/sst.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 sst module. Read this file when configuring or debugging the sst module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
Keeping with OpenSIPS, the module will not do anything to any message unless instructed to do so via the opensips.cfg script. You must set the sst\_flag value in the setflag() call of the INVITE you want the sst module to process. But before you can do that, you need to tell the sst module which flag value you are assigning to sst. In most cases when ever you create a new dialog via create\_dialog() function,you will want to set the sst flag. If create\_dialog() is not called and the sst flag is set, it will not have any effect. This parameter must be set of the module will not load.

*Default value is Not set!.*

**Notes:** This parameter must be set of the module will not load.
...
The sst minimum interval in Session-Expires header if OpenSIPS request the use of session times. The used value will be the maximum value between OpenSIPS minSE, UAS minSE and this value. Per default the interval used will be the min\_se value
...
Check the current Session-Expires / MIN-SE values against the sst_min_se parameter value. If the Session-Expires or MIN-SE header value is less then modules minimum value, this function will return true. If the fuction is called with the send_reply_flag set to true (1) and the requested Session-Expires / MIN-SE values are too small, a 422 reply will be sent for you. The 422 will carry a MIN-SE: header with the sst min_se parameter value set.
...
Set the enable_stats parameter to disable statistics support.
...
Set the min_se parameter to 2400 seconds.
...
Set the sst_interval parameter to 2400 seconds.
...
Set the reject_to_small parameter to 0 (false).
...
Set the sst_flag parameter and demonstrate its usage within a route block.
...
Demonstrates the usage of the sstCheckMin function within a route block.
