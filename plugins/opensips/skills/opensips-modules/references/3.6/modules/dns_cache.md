...
<!-- generated-from: data/3.6/modules/dns_cache.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 dns_cache module. Read this file when configuring or debugging the dns_cache module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `cachedb_*` — Must be loaded before loading the dns_cache module to provide the key-value back-end.
...
The minimum number of seconds that a DNS record will be kept in cache. If the TTL received in the DNS answer is lower than this value, the record will be cached for min_ttl seconds. _Default value is **0** seconds (no minimum TTL is enforced)._
...
Demonstrates setting the `cachedb_url` parameter to use either the internal cachedb_local module or a cachedb_memcached module.
...
Demonstrates setting the `blacklist_timeout` parameter to 7200 seconds.
...
Demonstrates setting the `min_ttl` parameter to 300 seconds.
