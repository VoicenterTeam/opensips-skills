...
<!-- generated-from: data/3.6/modules/json.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 json module. Read this file when configuring or debugging the json module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `dest_id` *(json, required)* — Destination JSON object.
- `source_id` *(json, required)* — Source JSON object.
...
- `main_json_var` *(string, required)* — The main JSON variable.
- `output_var` *(string, required)* — The variable to store the output.
- `patch_json_var` *(string, required)* — The patch JSON variable.
...
Setting the enable_long_quoting parameter and normalizing a value from $json.
...
Demonstrating setting values and accessing complex paths using the $json variable.
...
Iterating over an array, printing values, and incrementing them.
...
Iterating over the keys of a JSON object using the .keys suffix.
...
Iterating over the values of a JSON object using the .values suffix, and showing the equivalent iteration over the object itself.
...
Iterating over the values of a JSON array.
...
Appending an integer value to an array using the [] index and the = operator.
...
Deleting the last element of an array by setting it to NULL.
...
Adding a string key/value pair to a JSON object.
...
Initializing a JSON variable as an empty array using the := operator.
...
Setting boolean or null values into an array using the := operator to parse the string representation.
...
Assigning one JSON structure (array) to a field in another JSON structure (object) using the := operator to copy the structure.
...
Demonstrates creating a reference using json_link, modifying both the reference and the original, and then deleting the reference.
...
Demonstrates creating a circular reference using json_link, which is warned against as it causes a crash.
...
Demonstrates merging two JSON objects ($json(val2) into $json(val1)) and storing the result in a variable ($var(merged_json)).
