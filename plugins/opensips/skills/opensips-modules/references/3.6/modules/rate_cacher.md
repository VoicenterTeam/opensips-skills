...
<!-- generated-from: data/3.6/modules/rate_cacher.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 rate_cacher module. Read this file when configuring or debugging the rate_cacher module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `client_id` *(string, required)* — The client_id pseudo-var will hold the client_id originating this call
- `desired_margin` *(string, required)* — The desired_margin pseudo-var contains the minimum Integer margin that the script writer wants to achieve, based on the Client sell and Vendor buy prices. The formula used is : vendor_margin=(client_price - results[i])*100/client_price) . If the vendor_margin is higher than the desired_margin, then the Vendor is ok to use. The desired margin can be positive ( call will be profitable ) or negative ( the call will cause a loss ).
- `dialled_no` *(string, required)* — The dialled_no pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `is_wholesale` *(string, required)* — The is_wholesale pseudo-var will contain either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning ).
  - `1`
  - `0`
- `out_vendor_csv` *(string, required)* — The out_vendors_csv pseudo-var is an output parameter, and the pvar will get populated with the CSV list of Vendors that meet the desired margin condition
- `vendors_csv` *(string, required)* — The vendors_csv pseudo-var contains a list of Vendors that need to be filtered based on the desired margin ( keep just those that match your desired percentage margin for this call )
...
- `client_id` *(string, required)* — The client_id pseudo-var will hold the client_id originating this call
- `destination_pvar` *(string, required)* — The destination pseudo-var will contain the matched destination from the client's ratesheet
- `dialled_no` *(string, required)* — The dialled_no pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string, required)* — The increment pseudo-var will contain the matched increment from the client's ratesheet
- `is_wholesale` *(string, required)* — The is_wholesale pseudo-var will contain either a 1 or a 0, depending on whether the call is wholesale or retail ( see client ratesheet provisioning ).
  - `1`
  - `0`
- `minimum_pvar` *(string, required)* — The minimum pseudo-var will contain the matched minimum from the client's ratesheet
- `prefix_pvar` *(string, required)* — The prefix pseudo-var will contain the matched prefix from the client's ratesheet
- `price_pvar` *(string, required)* — The price pseudo-var will contain the matched price from the client's ratesheet
...
- `destination_pvar` *(string, required)* — The destination pseudo-var will contain the matched destination from the vendor's ratesheet
- `dialled_no` *(string, required)* — The dialled_no pseudo-var contains the DNIS - the dialled number for the current call. It needs to be in E164 format, without the leading +
- `increment_pvar` *(string, required)* — The increment pseudo-var will contain the matched increment from the vendor's ratesheet
- `minimum_pvar` *(string, required)* — The minimum pseudo-var will contain the matched minimum from the vendor's ratesheet
- `prefix_pvar` *(string, required)* — The prefix pseudo-var will contain the matched prefix from the vendor's ratesheet
- `price_pvar` *(string, required)* — The price pseudo-var will contain the matched price from the vendor's ratesheet
- `vendor_id` *(string, required)* — The vendor_id pseudo-var will hold the vendor_id
...
**Returns:** Ratesheet information including destination, price, minimum, increment, and currency (structured response — see schema)
