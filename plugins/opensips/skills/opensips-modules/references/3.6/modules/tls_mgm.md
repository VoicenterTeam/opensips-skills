...
<!-- generated-from: data/3.6/modules/tls_mgm.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 tls_mgm module. Read this file when configuring or debugging the tls_mgm module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
- `tls_openssl or tls_wolfssl` — The following modules must be loaded before this module, unless tls_library is set to 'none'.
...
**Example.** Set the `cipher_list_col` parameter.
...
You can specify the list of algorithms for authentication and encryption that you allow. The domain part represents the name of the TLS domain.
...
**Notes:** If there is already an existing TLS connection to the remote target, it will be reused and setting this AVP has no effect.

You can force a particular SIP domain to be used just for a particular branch by setting the _$bavp_ variable with the same name. When both _$bavp_ and _$avp_ variables are set, the first one takes precedence.

For the AVP usage example, refer to Section 1.9.36, “server_domain, client_domain (string)”.
...
**Notes:** By default, only the leaf certificate in the certificate chain is checked.
...
**Notes:** If this parameter is not set, no CRLs will be used.
...
**Example.** Set the `dh_params` parameter.
...
**Example.** Set the `dh_params_col` parameter.
...
**Example.** Set the `ec_curve_col` parameter.
...
**Example.** Set the `match_ip_address` parameter.
...
**Example.** Set the `match_sip_domain` parameter.
...
**Notes:** I must be kept in a safe place with tight permissions!
...
**Example.** Set the `require_cert` parameter.
...
Selects which TLS library to use. Possible values are: auto - auto-detect which TLS library module (_tls_openssl_ or _tls_wolfssl_) was loaded. OpenSIPS will not start if no module, or both modules are found. none - do not use any TLS library; this is useful when the _tls_mgm_ module is required only for the management of TLS certificates and parameters by modules like _db_mysql_, _rabbitmq_ etc. ( and not for TLS operations by transport modules like _proto_tls_ etc.) openssl - use the _openSSL_ library through the _tls_openssl_ module. wolfssl - use the _wolfSSL_ library through the _tls_wolfssl_ module.
...
**Example.** Set the `verify_cert` parameter.
...
Demonstrates checking if the peer was verified during TLS connection handshake.
...
Demonstrates setting the listen parameter for a TLS socket.
...
Demonstrates setting the tls_library parameter to 'none'.
...
Demonstrates setting the tls_method parameter for a specific domain.
...
Demonstrates setting the tls_method parameter using version ranges.
...
Demonstrates setting the certificate parameter for a specific domain.
...
Demonstrates setting the private_key parameter for a specific domain.
...
Demonstrates setting the ca_list parameter for a specific domain.
...
Demonstrates setting the ca_dir parameter for a specific domain.
...
Demonstrates setting the crl_dir parameter for a specific domain.
...
Demonstrates setting the crl_check_all parameter for a specific domain.
...
Demonstrates setting the ciphers_list parameter for a specific domain.
...
Demonstrates setting the dh_params parameter for a specific domain.
...
Demonstrates setting the verify_cert parameter for a specific domain.
...
Demonstrates setting the require_cert parameter for a specific domain.
...
Demonstrates setting the client_tls_domain_avp parameter.
...
Demonstrates setting the client_sip_domain_avp parameter.
...
Demonstrates setting the domain_col parameter.
...
Demonstrates setting the match_ip_address_col parameter.
...
Demonstrates setting the match_sip_domain_col parameter.
...
Demonstrates setting the tls_method_col parameter.
...
Demonstrates setting the verify_cert_col parameter.
...
Demonstrates setting the require_cert_col parameter.
...
Demonstrates setting the certificate_col parameter.
...
Demonstrates setting the private_key_col parameter.
...
Demonstrates setting the crl_check_all_col parameter.
...
Demonstrates setting the crl_dir_col parameter.
...
Demonstrates setting the ca_list_col parameter.
...
Demonstrates setting the ca_dir_col parameter.
...
Demonstrates setting the cipher_list_col parameter.
...
Demonstrates setting the dh_params_col parameter.
...
Demonstrates setting the ec_curve_col parameter.
...
Demonstrates setting the match_ip_address parameter for a specific domain.
...
Demonstrates setting the match_sip_domain parameter for specific domains.
...
Demonstrates defining and using tls_client_domain and tls_server_domain blocks with various matching rules.
...
...
```
### Example of `$tls_[peer|my]_[subject|issuer]`

Demonstrates the format of the $tls_[peer|my]_[subject|issuer] variable value.
...
Demonstrates a full OpenSIPS script configuration with TLS support.
...
Demonstrates logging all available TLS pseudo variables.
