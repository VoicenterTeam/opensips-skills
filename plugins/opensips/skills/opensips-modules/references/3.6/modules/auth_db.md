...
<!-- generated-from: data/3.6/modules/auth_db.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 auth_db module. Read this file when configuring or debugging the auth_db module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
**Notes:** Note: the default schema no longer comes with the rpid column, so if you are using the rpid value, you should provision it in your table. Otherwise, simply set this parameter to the empty string value.
...
**Notes:** IMPORTANT: before turning on this parameter, be sure that the `domain` column in `subscriber` table is properly populated.
...
- `true` — The username@domain from the given URI is an existing user
...
- `true` — The username extracted from the From header URI is allowed/authorized to use the credentials
...
- `true` — The username extracted from the To header URI is allowed/authorized to use the credentials
- `false` — Not authorized
...
- `-3` — Stale nonce
- `-2` — Valid user, but wrong password
- `-1` — Authentication user does not exist
