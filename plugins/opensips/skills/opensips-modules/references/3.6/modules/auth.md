...
<!-- generated-from: data/3.6/modules/auth.json
     generator-version: 0.1.0
     opensips-version: 3.6
     doc-type: module -->

Reference for the OpenSIPs 3.6 auth module. Read this file when configuring or debugging the auth module: signature, parameters, return codes, exported MI commands, statistics, events, and configuration examples.
...
This parameter tells the server whether it should expect plaintext passwords in the pseudo-variable or a pre-calculated HA1 string. If the parameter is set to 1 then the server will assume that the “password_spec” pseudo-variable contains plaintext passwords and it will calculate HA1 strings on the fly. If the parameter is set to 0 then the server assumes the pseudo-variable contains the HA1 strings directly and will not calculate them.
...
- `prefix` *(string, required)* — Prefix of the Remote-Party-ID URI. The string will be added at the beginning of body of the header field, just before the URI.
- `suffix` *(string, required)* — Suffix of the Remote-Party-ID header field. The string will be appended at the end of the header field. It can be used to set various URI parameters, for example.
...
**Example.** append_rpid_hf(prefix, suffix) usage.
...
**Example.** consume_credentials example.
...
**Example.** is_rpid_user_e164 usage.
...
- `algorithms` *(string, optional)* — Value of this parameter is a comma-separated list of digest algorithms to be offered for the UAC to use for authentication. When the value is empty or not set, the only offered digest algorithm is _MD5_, to provide compatibility with pre-RFC8760 UAC implementations. Values can be listed in any order. The actual order of individual challenges in SIP response is defined by the RFC8760: from stronger algorithm to a weaker one.
  - `MD5`
  - `MD5-sess`
  - `SHA-256`
  - `SHA-256-sess`
...
**Example.** pv_proxy_authorize usage.
...
**Example.** pv_www_authorize usage.
...
Demonstrates setting the secret parameter.
...
Demonstrates setting the nonce_expire parameter.
...
Demonstrates setting the rpid_prefix parameter.
...
Demonstrates setting the rpid_suffix parameter.
...
Demonstrates setting the realm_prefix parameter.
...
Demonstrates setting the rpid_avp parameter.
...
Demonstrates setting the username_spec parameter.
...
Demonstrates setting the password_spec parameter.
...
Demonstrates setting the calculate_ha1 parameter.
...
Demonstrates setting the disable_nonce_check parameter.
...
Demonstrates usage of the www_challenge function.
...
Demonstrates usage of the proxy_challenge function.
...
Demonstrates usage of the consume_credentials function.
...
...
```
### is_rpid_user_e164 usage

Demonstrates usage of the is_rpid_user_e164 function.
...
Demonstrates usage of the append_rpid_hf function.
...
Demonstrates usage of the append_rpid_hf function with parameters.
...
Demonstrates usage of the pv_www_authorize function.
...
...
```
### pv_proxy_authorize usage

Demonstrates usage of the pv_proxy_authorize function.
