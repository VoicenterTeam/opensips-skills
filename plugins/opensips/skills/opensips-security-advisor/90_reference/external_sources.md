...
- **The "Phase 5 citation sweep" the original brief described** is not represented here. That sweep would have replaced `[P5 §TBD — ...]` placeholders in spec docs with section references into `03_advisor_methodology_research.md`. Since `03_*` is currently a stub and the spec docs in question (per the brief, "Chat A's 14 spec docs") are not in this bundle's scope, the sweep is N/A. Future consolidation passes that locate Chat A's docs should run the sweep separately.
