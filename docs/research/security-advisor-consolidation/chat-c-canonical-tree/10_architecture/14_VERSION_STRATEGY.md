...
# OR-list — multiple disjoint ranges (multi-branch CVE rules)
...
- **Whitespace inside a sub-range forms an AND** (intersection):
  `">=3.4 <3.7"` matches versions ≥3.4 AND <3.7.
- **`||` between sub-ranges forms an OR** (disjunction): any
  matching sub-range satisfies the predicate. The OR-list form
  is the canonical encoding for CVE-gated rules where the fix
  has been backported to multiple LTS branches with different
  patched-in version numbers — for example, a CVE patched in
  3.4.7, 3.5.4, and 3.6.4 is encoded as
  `">=3.4.0 <3.4.7 || >=3.5.0 <3.5.4 || >=3.6.0 <3.6.4"`. This
  keeps one rule per CVE rather than three branch-specific
  rules. Backport release numbers must be confirmed before they
  enter the OR-list; don't speculate (see Tier 4 version overlay
  docs for the current per-branch patched-in registry).
- An invalid range fails catalog validation
  (`12_RULE_CATALOG_SCHEMA.md` validation rule 9). The validator
  accepts both single-range and OR-list forms.
