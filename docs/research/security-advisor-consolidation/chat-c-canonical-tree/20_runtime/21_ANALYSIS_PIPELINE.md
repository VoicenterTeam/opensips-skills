...
**CVSS aggregation across findings.** When the report layer needs
a single CVSS score for a roll-up — per-module, per-section, or a
single headline number for the cfg — the policy is
**worst-case-across-modules**: the aggregate score is the maximum
`security_severity` (and the corresponding vector) of any
contributing finding within the scope of the roll-up. Module
families are not averaged, and high-frequency lower-severity
findings do not raise the aggregate above the maximum. The
rationale is that a deployment is exposed at the level of its
worst un-mitigated weakness; averaging would let a single
critical finding be diluted by many `info`-grade findings, which
contradicts how operators triage. Per-finding CVSS vectors are
preserved unchanged in the SARIF and Markdown outputs; the
aggregate is a derived view, not a replacement for per-finding
scores.
